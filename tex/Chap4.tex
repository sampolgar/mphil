...
\section{Chapter Roadmap and Contributions}
...
\begin{enumerate}
    ...
    \item \textbf{Private Proof of Inverse Exponent:} The rest of the work focuses on privacy.  My first step in section \ref{sec:vrf-prime-order} is a new $\Sigma$-protocol to anonymously prove the DDHI challenge $(g^{1/x})$, which I generalise to a sigma protocol for \emph{Private Proof of Inverse Exponent}.

    \item \textbf{Deterministic Nullifier:} In Section \ref{sec:privacy-preserving-vrf} I combine the \emph{Private Proof of Inverse Exponent} with the \emph{Proof of Inverse Linear Relation} to now anonymously prove the prime-order DY, which is the output of my deterministic nullifier. 

    \item \textbf{Provable Probabilistic Nullifier:} I use a similar but more complicated protocol to generate a provable probabilistic nullifier and explain its use cases in revocation schemes.
\end{enumerate}
...
% \begin{tikzpicture}[
%     node distance=1cm,
%     box/.style={rectangle, rounded corners, draw, fill=white, text width=2.5cm, minimum height=1.5cm, align=center, font=\small},
%     arrow/.style={thick,->,>=stealth},
%     improvement/.style={text width=3cm, align=center, font=\footnotesize\itshape}
% ]
% % Root node
% \node[box, fill=gray!10] (ddhi) {\textbf{q-DDHI Assumption}};

% % Branch 1 - Top horizontal branch
% \node[box, right=1.25cm of ddhi, yshift=1cm] (pfdy) {\textbf{My Improved DY VRF}};

% % Branch 2 - Bottom horizontal branch with sequence
% \node[box, right=1.25cm of ddhi, yshift=-1cm] (zkpok) {\textbf{ZKPoK of q-DDHI challenge}};
...
\subsection{My Sigma-Protocol: Proving Knowledge of a Committed Inverse Linear Relation}\label{sec-pok-committed-inv-linear-relation}
...
\subsection{New Probabilistic Nullifier from \ref{chap4:pok_committed_inverse_linear_relation}}\label{sec-probabilistic-nullifier}
...
% Introducing the nullifier and its cryptographic role
...
% Explaining Sybil resistance with nullifiers
...
% Detailing revocation with committed nullifiers
...
% Describing credential management in the wallet
...
% Summarizing the nullifier's role and efficiency
