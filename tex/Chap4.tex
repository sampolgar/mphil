\mychapter{Efficient Relationship Binding Nullifiers}\label{chap4}
...
\begin{figure}
    ...
    \node[font=\small, align=center] at ($(userlist.north) + (0,-1.3)$) {Sec: \ref{sec-deterministic-nullifier} $ y = g^{1/(k + \text{"DMV"})}$};
...
Sigma+Pairing & 4x slower 
\\
\text{Sec:} \ref{sec-deterministic-nullifier}: & 
\ding{51} & 
\ding{55} & 
\ding{51} & 
\ding{51} & 
...
\subsection{Deterministic Nullifier VRF}\label{sec-deterministic-nullifier}
