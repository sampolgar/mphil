...
\subsection{New Zero-Knowledge Proof of Knowledge of Inverse Linear Relation}\label{sec-pok-committed-inv-linear-relation}
...
\begin{protocol}{Proof of Knowledge of Inverse Linear Relation}{}\label{pok-committed-inverse-linear-relation}
...
\begin{protocol}{Proof of Knowledge of Committed Nullifier}{}
...
%     \item \textbf{Unlinkable Output for Prime-Order VRF}: The deterministic nullifier can be used for sybil resistance during credential generation but can't be used ongoing as each output links the user's actions. Our fourth and last core challenge is to extend our protocol to generate provable, probabilistic nullifiers. We need to output a commitment to the nullifier so it can be used without linking the user. The final protocol proves knowledge of $(x, s,r)$ such that $\cm_y = g_1^{1/(sk + x)}g^r$. 
% \end{enumerate}
...
% \textbf{Relation: }
% \[
% \mathcal{R} = \{(\cm), (s, x, \usk) \mid \cm = g_1^{sk}g_2^{x}g_3^{sk + x}g^{\usk}\}
% \]
% \textbf{Common Input:} Generator $g_1,g_2,g_3, g \in \G$, public elements $\cm \G$\\
% \textbf{Prover Input:} Witness $(s, x, \usk)$
% \begin{enumerate}
...
%     \item \textbf{Response:} Prover computes:
%     \[
%     z_{x} = a_x + c \cdot x \qquad z_{s} = a_{sk} + c \cdot sk \qquad z_{\usk_1} = a_r + c \cdot \usk
%     \]
%     Sends $z_{x}, z_{s}, z_{r}$ to verifier
    
%     \item \textbf{Verification:} Verifier checks:
%     \[
%     \cm^c \cdot T \stackrel{?}{=} g_1^{z_{x}} g_2^{z_{s}} g_3^{(z_x + z_{s}}) g^{z_{\usk_1}}
%     \]
% \end{enumerate}
% \end{protocol}
...
% Definition: 
% - PLUME: nullifier N(s,pk) for ECDSA (s,pk), and some app-specific message $m$ can also include CRS. Correctness, for all $pk_i, m, Ver$; $Ver(N(m,sk_i,pk_i), proof_j , m, pk_i)) = pk_i$
