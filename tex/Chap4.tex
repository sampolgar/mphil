\chapter{New Nullifier Constructions from the q-DDHI and Applications to Accountable Privacy Systems }\label{chap4}
...
\begin{figure}
    ...
    \scalebox{0.85}{ % Scale down the entire figure
        ...
        \node[anchor=north, font=\bfseries\small] at ($(wallet.north) + (0,+0.3)$) {Digital Credential Wallet};
        ...
        \node[font=\footnotesize, align=center] at ($(nullifier.north) + (0,-0.8)$){Sec: \ref{subsec:deterministic-nullifier}};
        \node[font=\footnotesize, align=center] at ($(nullifier.north) + (0,-1.3)$) {$y = g^{1/(k + \text{"DMV"})}$};
        
        \node[font=\footnotesize, align=center] at ($(nullifier.north) + (0,-1.8)$) {Sec: \ref{sec-probabilistic-nullifier}};
        \node[font=\footnotesize, align=center] at ($(nullifier.north) + (0,-2.3)$) {$\cm_y = g_1^{1/(k + \text{"DMV"})}g^{\usk_3}$};
        
        % Repositioned list boxes at least 1cm away from nullifier
        \node[listbox] (userlist) at (11,0.7) {}; % Increased distance to at least 1cm
        \node[anchor=north, font=\bfseries\small] at ($(userlist.north) + (0,-0.3)$) {Issued List};
        
        % Content positioned lower
        \node[font=\footnotesize, align=center] at ($(userlist.center) + (0,0.1)$) {$y$}; % Slight adjustment
        
        % Revocation List - repositioned
        \node[listbox] (revlist) at (11,-0.7) {}; % Increased distance to at least 1cm
        \node[anchor=north, font=\bfseries\small] at ($(revlist.north) + (0,-0.3)$) {Revocation List};
        
        % Content positioned lower
        \node[font=\footnotesize, align=center] at ($(revlist.center) + (0,0.1)$) {$\cm_{y,1}$}; % Slight adjustment
...
\begin{figure}
    ...
    \scalebox{0.85}{ % Scale down the entire figure
        ...
        % Centered nullifier text with better spacing
        \node[font=\footnotesize, align=center] at ($(nullifier.north) + (0,-0.8)$) {Sec: 4.5.4};
        \node[font=\footnotesize, align=center] at ($(nullifier.north) + (0,-1.3)$) {$y = g^{1/(k + \text{"DMV"})}$};
        
        \node[font=\footnotesize, align=center] at ($(nullifier.north) + (0,-1.8)$) {Sec: 4.5.5};
        \node[font=\footnotesize, align=center] at ($(nullifier.north) + (0,-2.3)$) {$\cm_y = g_1^{1/(k + \text{"DMV"})}g^{\usk_3}$};
...
\begin{figure}
    ...
    \scalebox{0.85}{ % Scale down the entire figure
        ...
        % Rearranged nullifier text with better spacing
        \node[font=\footnotesize, align=left] at ($(nullifier.north) + (-1.8,-0.8)$) {Sec: 4.5.4};
        \node[font=\footnotesize, align=left] at ($(nullifier.north) + (0,-1.1)$) {$y = g^{1/(s + \text{"DMV"})}$};
        
        \node[font=\footnotesize, align=left] at ($(nullifier.north) + (-1.8,-1.7)$) {Sec: 4.5.5};
        \node[font=\footnotesize, align=left] at ($(nullifier.north) + (0,-2.0)$) {$\cm_y = g_1^{1/(s + \text{"DMV"})}g^{\usk_3}$};
        
        % Repositioned list boxes to be more in line with the nullifier box
        \node[listbox] (userlist) at (10,0.7) {}; % More horizontally aligned
        \node[anchor=north, font=\bfseries\small] at ($(userlist.north) + (0,-0.3)$) {Issued List};
        
        % Just add the content without lines
        \node[font=\footnotesize, align=center] at ($(userlist.center)$) {$y$};
        
        % Revocation List - repositioned
        \node[listbox] (revlist) at (10,-0.7) {}; % More horizontally aligned
        \node[anchor=north, font=\bfseries\small] at ($(revlist.north) + (0,-0.3)$) {Revocation List};
        
        % Just add the content without lines
        \node[font=\footnotesize, align=center] at ($(revlist.center)$) {$\cm_{y,1}$};
        
        % Straighter arrows
        \draw[->, thick] (wallet.east) -- (nullifier.west);
        \draw[->, thick] (nullifier.east) -- ($(nullifier.east)!0.25!(userlist.west)$) -- ($(nullifier.east)!0.25!(userlist.west) + (0,0.7)$) -- (userlist.west); % More direct
        \draw[->, thick] (nullifier.east) -- ($(nullifier.east)!0.25!(revlist.west)$) -- ($(nullifier.east)!0.25!(revlist.west) + (0,-0.7)$) -- (revlist.west); % More direct
...
\begin{figure}
    ...
    \scalebox{0.85}{ % Scale down the entire figure
        \begin{tikzpicture}[
            box/.style={draw, rounded corners=2pt, minimum width=4.5cm, minimum height=6cm}, % Reduced width
            smallbox/.style={draw, rounded corners=2pt, fill=gray!10, minimum width=3.8cm, minimum height=2.2cm}, % Adjusted size
            listbox/.style={draw, rounded corners=2pt, minimum width=3.5cm, minimum height=1.5cm}, % Keeping size but removing internal lines
            nullifierbox/.style={draw, rounded corners=2pt, fill=gray!10, minimum width=5cm, minimum height=2.5cm}, % Keeping as is
            thick
        ]
        ...
        % Nullifier in the middle
        \node[nullifierbox] (nullifier) at (5.5,0) {}; % Slightly adjusted position
        \node[anchor=north, font=\bfseries\small] at ($(nullifier.north) + (0,0.4)$) {Nullifier Generation};
        
        % Add the deterministic nullifier algorithm with section reference
        \node[font=\footnotesize, align=center] at ($(nullifier.north) + (0,-1.3)$) {Sec: \ref{subsec:deterministic-nullifier} $y = g^{1/(s + \text{"DMV"})}$};
        
        % Add the probabilistic nullifier algorithm with section reference
        \node[font=\footnotesize, align=center] at ($(nullifier.north) + (0,-2.0)$) {Sec: \ref{sec-probabilistic-nullifier} $\cm_y = g_1^{1/(s + \text{"DMV"})}g^{\usk_3}$};
        
        % Issued List - simplified
        \node[listbox] (userlist) at (10,1.6) {}; % Adjusted position
        \node[anchor=north, font=\bfseries\small] at ($(userlist.north) + (0,-0.3)$) {Issued List};
        
        % Just add the content without lines
        \node[font=\footnotesize, align=center] at ($(userlist.center)$) {$y$};
        
        % Revocation List - simplified
        \node[listbox] (revlist) at (10,-1.6) {}; % Adjusted position
        \node[anchor=north, font=\bfseries\small] at ($(revlist.north) + (0,-0.3)$) {Revocation List};
        
        % Just add the content without lines
        \node[font=\footnotesize, align=center] at ($(revlist.center)$) {$\cm_{y,1}$};
        
        % Arrows - update to come from center of nullifier
        \draw[->, thick] (wallet.east) -- (nullifier.west);
        \draw[->, thick] (nullifier.east) -- ++(0.5,0) -- ++(0,1.6) -- (userlist.west); % From middle to Issued List
        \draw[->, thick] (nullifier.east) -- ++(0.5,0) -- ++(0,-1.6) -- (revlist.west); % From middle to Revocation List
...
\begin{figure}
    \centering
    \begin{tikzpicture}[
        box/.style={draw, rounded corners=2pt, minimum width=5cm, minimum height=6cm},
        smallbox/.style={draw, rounded corners=2pt, fill=gray!10, minimum width=4cm, minimum height=2.5cm},
        listbox/.style={draw, rounded corners=2pt, minimum width=3cm, minimum height=3cm},
        nullifierbox/.style={draw, rounded corners=2pt, fill=gray!10, minimum width=4.7cm, minimum height=2.5cm},
        thick
    ]
    
    % Digital Credential Wallet
    \node[box] (wallet) at (0,0) {};
    \node[anchor=north, font=\bfseries] at ($(wallet.north) + (0,+0.5)$) {Digital Credential Wallet};
    ...
    % Nullifier in the middle
    \node[nullifierbox] (nullifier) at (6,0) {};
    \node[anchor=north, font=\bfseries] at ($(nullifier.north) + (0,-0.3)$) {Nullifier Generation};
    
    % Add the deterministic nullifier algorithm with section reference
    \node[font=\small, align=center] at ($(nullifier.north) + (0,-1.3)$) {Sec: \ref{subsec:deterministic-nullifier} $y = g^{1/(k + \text{"DMV"})}$};
    
    % Add the probabilistic nullifier algorithm with section reference
    \node[font=\small, align=center] at ($(nullifier.north) + (0,-2.0)$) {Sec: \ref{sec-probabilistic-nullifier} $\cm_y = g_1^{1/(k + \text{"DMV"})}g^{\usk_3}$};
    
    % Anonymous User List
    \node[listbox] (userlist) at (11,1.6) {};
    \node[anchor=north, font=\bfseries] at ($(userlist.north) + (0,-0.3)$) {Issued List};
    
    % Draw horizontal lines in the user list
    \draw ($(userlist.north west) + (0,-0.8)$) -- ($(userlist.north east) + (0,-0.8)$);
    
    % Reference to section in the user list but no formula
    \node[font=\small, align=center] at ($(userlist.north) + (0,-1.3)$) {$y$};
    ...
    \node[font=\small, align=center] at ($(revlist.north) + (0,-1.3)$) {$\cm_{y,1}$};
    ...
    % Arrows - update flow through nullifier
    \draw[->, thick] (wallet.east) -- (nullifier.west);
    \draw[->, thick] (nullifier.north east) -- (userlist.west);
    \draw[->, thick] (nullifier.south east) -- (revlist.west);
...
\todonote{sam needs to update this}

\section{We Formally Define a Nullifier}

\begin{definition}[Nullifier Scheme]
...
\begin{property}[Existential Unforgeability]
