...
\[ \Pr \left[\begin{array}{l}
    ...
    (\top, 1) \sample (Show(opk, φ, \{\mathsf{cred}_i\}_{i \in [n]}),\\
    \quad Verify(opk, φ))
\end{array}\right] = 1 \]
\end{definition}

\begin{definition}[Correctness]
A Multi-Issuer Multi-Credential Attribute based Anonymous Credential System (MIMC-ABC) is correct if for all $k > 0$, all $t > 0$ and all $A$
A Multi-Issuer Multi-Credential Anonymous Credential system (MIMC-ABC) is correct if for all κ > 0, all t > 0 and all 
\[ \Pr \left[\begin{array}{l}
    (osk_i, opk_i) \sample OrgKeyGen(1^k, 1^t) \text{ for } i \in [n],\\
    (usk, upk) \sample UserKeyGen(opk),\\
    (\mathsf{cred}_i, \top) \sample (Obtain(usk, opk_i, \mathbf{A}_i),\\
    \quad Issue(upk, osk_i, \mathbf{A}_i)) \text{ for } i \in [n],\\
    (\top, 1) \sample (Show(opk, φ, \{\mathsf{cred}_i\}_{i \in [n]}),\\
    \quad Verify(opk, φ))
\end{array}\right] = 1 \]
\end{definition}
