...
\begin{itemize}
    ...
    \]
        ...
        \item $\mathsf{RS.VerKey}(\sk, \vk, \ck) \to \bit:$ checks $e(\sk, \vk) = 1\G_T$ outputs 1 for success or 0 for failure. 
\end{itemize}
...
\begin{definition}[MIMC-ABC System] A Multi-Issuer Multi-Credential Attribute-based Anonymous Credential system consists of the following $\PPT$ algorithms:
    ...
    \item $\mathsf{UKeygen}(\ppar) \to (\usk)$: is a probabilistic algorithm that takes public parameters $\ppar$, outputs users secret key $\usk$
    
    \item $\mathsf{UserCommit}(\opk, \vec{m}, \usk) \rightarrow (\cm)$ is a probabilistic algorithm that takes as input the organization's public key $\opk$, a vector of messages $\vec{m}$ and the user's secret key $\usk$ which will be the randomness in the commitment. It outputs a commitment $\cm = \CMCom([\vec{m}];\usk)$.

    \item $(\mathsf{Obtain}(\cm, \usk, \opk,  \phi, aux), \mathsf{Issue}(\osk, \cm, \phi, \{\Pi\}_i, aux)) \rightarrow (\cred, \bot)$ is an interactive protocol between a user and an issuing organization. Can be used 
    for obtaining a master or context credential $\credm, \credc$. $\mathsf{Obtain}$ The user inputs their commitment $\cm$ and $\usk$ and generates a proof of knowledge satisfying statement $\phi$. Obtaining a Context Credential requires verifying a master credential and consistency of user id between master and context which we represent as auxiliary information $aux$. $\mathsf{Issue}$ Issuer runs this with their secret key $\osk$, commitment $\cm$, verifies proofs $\{\Pi\}_i$ against the statement $\phi$. 
...
\begin{figure}
    ...
    \multicolumn{3}{l}{$\mathsf{(Obtain, Issue)}: \cmm = \CMCom([0,0];r), \pirzero(\cmm) = \zkpok\{(\cmm, r)| \cmm = g_1^0g_2^0g^r\}$}\\[1em]
