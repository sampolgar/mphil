...
\begin{definition}[MIMC-ABC System] A Multi-Issuer Multi-Credential Attribute based Anonymous Credential system consists of the following $\PPT$ algorithms:
    ...
    \item $(\mathsf{Obtain Context}(\mathsf{cm_c}, r_c, \mathsf{cred_m}, aux), \mathsf{Issue Context}(\mathsf{opk_m}, \mathsf{cred_m}, \mathsf{osk_c}, \mathsf{cm_c}, aux)) \to \mathsf{cred_c}:$ An interactive protocol between a user and an issuing organization. $Obtain Context$ is a probabilistic algorithm run by a user, inputs master credential $Cred_m$ and corresponding organisation public key $opk_m$ for verification. Also inputs $\mathsf{cm_c}, r_c$ the new committed values and randomness. 
    $\mathsf{Issue Context}$ s a probabilistic algorithm run by an issuing organization that takes a commitment $cm$, issuer secret key $osk_c$, and auxiliary info. Outputs a credential $Cred_c$ binding $cm_c$ to the issuer signature.
...
\noindent $\mathcal{O}_{\mathsf{HU}}(i)$ - By calling this honest user oracle with identity $i \in \mathbb{N}$, $\mathcal{A}$ can add $i$ to the set $\mathsf{HU}$ of honest users. The oracle 


The oracle generates $(\mathsf{usk}[i], \mathsf{upk}[i]) \sample \mathsf{UserKeyGen}(\mathsf{opk})$, adds $i$ to $\mathsf{HU}$, and returns $\mathsf{upk}[i]$.
