...
We write $A(x,y,\ldots : \oracle_1, \oracle_2, \ldots)$ as an algorithm taking $(x,y,\ldots)$ as input with access to oracles $(\oracle_1, \oracle_2, \ldots)$ and $z \sample A(x,y,\ldots : \oracle_1, \oracle_2, \ldots)$ as the assignment of the output of $A$ to $z$. We denote the adversary algorithm as $\Adv$
...
\begin{definition}[Binding] A rerandomizable vector commitment scheme $\mathsf{RVC}$ is binding if for all $\PPT$ adversaries $\Adv$ and all message vectors $\vec{m}_0, \vec{m}_1$ of equal length, there is a negligible function $\negl$ such that:
    \[
        ...
        \left [
            \begin{array}{l}
            \mathsf{ck} \gets \mathsf{Setup}(1^\lambda, 1^n) \\
            (\vec{m}_0, \vec{m}_1, r_0, r_1) \gets \Adv(\mathsf{ck}) \\
...
\[
    ...
    \right] \leq \negl(\lambda)
\]
Informally, this states that an adversary $\Adv$ should not be able to open a commitment $\cm$ to index $i$ unless both commitments use the same message, even when the commitments only differ by position $i$
\end{definition}
...
Proof Structure
1. Start with a position binding adversary A
2. Build a reduction B that solves SDLP
3. Show that if A breaks position binding with Pr > than negl, then B solves SDLP with Pr > negl

For the SDLP challenge
$g_1 = g^x, \tilde{g}_1 = \tilde{g}^x$
Compute For $i \in [2, \ell], y_i \sample \Z_p, g_i \gets g^{y_i}, \tilde{g}_i \gets \tilde{g}^{y_i}$
For $\Adv$ to break position binding, they claim they found different openings for the same position of the message. 
$\cm = g^r$
