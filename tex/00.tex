...
% \begin{theorem}[EUF-CCA Security]
% In the Algebraic Group Model, if the PS-LRSW assumption holds in the bilinear group $\mathbb{G}$, our signature scheme is existentially unforgeable under chosen message attacks. Specifically, for any algebraic PPT adversary $\mathcal{A}$ making at most $q_s$ signing queries and $q_v$ verification queries, there exists a PPT reduction $\mathcal{B}$ against PS-LRSW such that:
% \[
%     \Adv^{\mathsf{euf\mbox{-}cca}}_{\mathsf{PS},\mathcal{A}}(\lambda) \leq \Adv^{\mathsf{PS\mbox{-}LRSW}}_{\mathcal{B}}(\lambda) + \frac{q_v}{p}
% \]
% \end{theorem}
...
% \begin{proof}
% We proceed through a sequence of games, showing that any successful forgery in our scheme would lead to breaking the PS-LRSW assumption.

% Game 0: The original EUF-CCA security game where adversary $\mathcal{A}$ has access to signing oracle $\mathcal{O}_{\mathsf{Sign}}$ and verification oracle $\mathcal{O}_{\mathsf{Ver}}$.

% Game 1: We modify Game 0 by replacing the verification oracle with an algebraic verification. Since $\mathcal{A}$ is algebraic, for any group element $Z$ it queries to $\mathcal{O}_{\mathsf{Ver}}$, it must provide its representation in terms of the generators:
% \[
% Z = g^{\alpha_0}\prod_{i=1}^n g_i^{\alpha_i}h^{\alpha_{n+1}}
% \]
% where $\{g, g_1,\ldots,g_n, h\}$ are the public generators.

% Let $(\sigma^*, \cm^*)$ be $\mathcal{A}$'s forgery attempt. Since $\mathcal{A}$ is algebraic, it provides representations:
% \begin{align*}
% \sigma_1^* &= g^{\beta_0}\prod_{i=1}^n g_i^{\beta_i}h^{\beta_{n+1}} \\
% \sigma_2^* &= g^{\gamma_0}\prod_{i=1}^n g_i^{\gamma_i}h^{\gamma_{n+1}}
% \end{align*}

% Game 2: In this game, we extract the algebraic structure of the forgery. Given $\mathcal{A}$'s forgery $(\sigma^*, \cm^*)$ with algebraic representations:
% \begin{align*}
% \sigma_1^* &= g^{\beta_0}\prod_{i=1}^n g_i^{\beta_i}h^{\beta_{n+1}} \\
% \sigma_2^* &= g^{\gamma_0}\prod_{i=1}^n g_i^{\gamma_i}h^{\gamma_{n+1}} \\
% \cm^* &= g^{\delta_0}\prod_{i=1}^n g_i^{\delta_i}h^{\delta_{n+1}}
% \end{align*}

% For a valid forgery, the verification equation must hold:
% \[
% e(\sigma_2^*, \tilde{g}) = e(\sigma_1^*, \tilde{X} \cdot \tilde{Y}^{m^*})
% \]

% By substituting the algebraic representations and using the bilinearity of the pairing, we obtain:
% \begin{align*}
% e(g^{\gamma_0}\prod_{i=1}^n g_i^{\gamma_i}h^{\gamma_{n+1}}, \tilde{g}) &= e(g^{\beta_0}\prod_{i=1}^n g_i^{\beta_i}h^{\beta_{n+1}}, \tilde{g}^x \cdot \tilde{g}^{ym^*}) \\
% \end{align*}

% This yields a system of equations in the exponents:
% \begin{align*}
% \gamma_0 &= \beta_0(x + ym^*) \\
% \gamma_i &= \beta_i(x + ym^*) \text{ for } i \in [1,n] \\
% \gamma_{n+1} &= \beta_{n+1}(x + ym^*) 
% \end{align*}

% Game 3: We construct our reduction $\mathcal{B}$ to PS-LRSW. $\mathcal{B}$ receives a PS-LRSW challenge $(g, \tilde{g}, X = g^x, Y = g^y, \tilde{X} = \tilde{g}^x, \tilde{Y} = \tilde{g}^y)$ and simulates the EUF-CCA game for $\mathcal{A}$ as follows:

% 1. Setup: $\mathcal{B}$ forwards the generators and challenge elements to $\mathcal{A}$

% 2. Signing Queries: For each signing query on commitment $\cm_i$, $\mathcal{B}$ queries its PS-LRSW oracle to obtain a signature pair $(h_i, h_i^{x+m_iy})$

% 3. Verification Queries: $\mathcal{B}$ uses the algebraic representations provided by $\mathcal{A}$ to verify signatures

% When $\mathcal{A}$ outputs a valid forgery $(\sigma^*, \cm^*)$, $\mathcal{B}$ uses the algebraic representations to extract:
% \[
% h^* = \sigma_1^* = g^{\beta_0}\prod_{i=1}^n g_i^{\beta_i}h^{\beta_{n+1}}
% \]
% \[
% (h^*)^{x+m^*y} = \sigma_2^* = g^{\gamma_0}\prod_{i=1}^n g_i^{\gamma_i}h^{\gamma_{n+1}}
% \]

% Since $\cm^*$ was never queried to the signing oracle, this constitutes a valid PS-LRSW forgery.



% Let us analyze the probability of $\mathcal{A}$'s success in each game. Let $\mathsf{Succ}_i$ denote the event that $\mathcal{A}$ succeeds in Game i.

% \paragraph{Game 0 → Game 1:}
% The transition from Game 0 to Game 1 is purely conceptual. We simply require $\mathcal{A}$ to provide algebraic representations of group elements, which any algebraic adversary must be able to do by definition. Therefore:
% \[
% \Pr[\mathsf{Succ}_0] = \Pr[\mathsf{Succ}_1]
% \]

% \paragraph{Game 1 → Game 2:}
% In Game 2, we extract the algebraic structure of $\mathcal{A}$'s forgery. This is again a conceptual change that doesn't affect $\mathcal{A}$'s view of the game:
% \[
% \Pr[\mathsf{Succ}_1] = \Pr[\mathsf{Succ}_2]
% \]

% \paragraph{Game 2 → Game 3:}
% This transition introduces our reduction to PS-LRSW. We need to show that $\mathcal{B}$'s simulation is perfect unless a specific event occurs. Let $\mathsf{Bad}$ be the event that $\mathcal{A}$ produces a verification query $(\sigma, \cm)$ that verifies correctly but doesn't correspond to a valid algebraic representation.

% By the properties of the algebraic group model, this can only occur if $\mathcal{A}$ finds a collision in the pairing equations. Given the q-type assumption underlying PS-LRSW, this probability is bounded by $q_v/p$ where $q_v$ is the number of verification queries:
% \[
% |\Pr[\mathsf{Succ}_2] - \Pr[\mathsf{Succ}_3]| \leq \Pr[\mathsf{Bad}] \leq \frac{q_v}{p}
% \]

% \paragraph{Final Analysis:}
% If $\mathcal{A}$ succeeds in Game 3, then $\mathcal{B}$ successfully breaks the PS-LRSW assumption. Therefore:
% \[
% \Pr[\mathsf{Succ}_3] \leq \Adv^{\mathsf{PS\mbox{-}LRSW}}_{\mathcal{B}}(\lambda)
% \]

% Combining all the probability bounds:
% \begin{align*}
% \Adv^{\mathsf{euf\mbox{-}cca}}_{\mathsf{PS},\mathcal{A}}(\lambda) &= \Pr[\mathsf{Succ}_0] \\
% &= \Pr[\mathsf{Succ}_1] \\
% &= \Pr[\mathsf{Succ}_2] \\
% &\leq \Pr[\mathsf{Succ}_3] + \frac{q_v}{p} \\
% &\leq \Adv^{\mathsf{PS\mbox{-}LRSW}}_{\mathcal{B}}(\lambda) + \frac{q_v}{p}
% \end{align*}

% This completes the proof, showing that any algebraic adversary's advantage in breaking EUF-CCA security is bounded by their advantage in breaking PS-LRSW plus a negligible term from verification queries.
% \end{proof}
