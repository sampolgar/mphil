...
\begin{enumerate}
    ...
    \item Challenge Phase: $\mathcal{A}$ outputs $(\{\mathsf{cred}_i\}_{i \in [k]}, \phi, \pi)$

    \item The experiment returns 1 if:
    \begin{itemize}
        \item $\mathsf{Verify}(opk, \phi, \{\mathsf{cred}_i\}_{i \in [k]}, \pi) = 1$ $\wedge$
        \item At least one of the following holds:
            \begin{enumerate}
                \item $\exists i,j: \mathsf{OWNR}(\mathsf{cred}_i) \neq \mathsf{OWNR}(\mathsf{cred}_j)$ (inconsistent ownership)
                \item $\exists i: \mathsf{cred}_i \notin \mathsf{CRED}$ (credential forgery)
            \end{enumerate}
    \end{itemize}
\end{enumerate}
