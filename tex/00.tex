...
Zero-Knowledge Proofs (ZKPs) enable a prover $\Prover$ to demonstrate the validity of a statement $x$ while keeping the witness $w$ secret. In anonymous credentials, ZKPs allow a user to prove possession of a credential, validity of its attributes, and comply with policies while revealing none or minimal sensitive underlying data. 

Signpost
1. Introduce Zero Knolwedge Proofs
2. Present the core protocols
3. 

\subsection{Interactive Proof Systems, the Prover $\Prover$ and Verifier $\Verifier$}
An interactive proof system for a language L consists of two probabilistic interactive Turing machines $(\Prover, \Verifier)$, where $\Prover$ is the prover and $\Verifier$ is the verifier, in Anonymous Credentials, $\Prover$ is a user with a credential signed by an Issuer, $\Verifier$ is any public system wanting to verify the user's identity and attributes. The system must satisfy:
\begin{itemize}
...
While soundness ensures the validity of statements,  many cryptographic applications require stronger guarantees about the prover's knowledge. A proof of knowledge introduces the concept of an extractor $\mathcal{E}$ that, given rewindable access to any prover $\Prover^*$ that succeeds with non-negligible probability, can efficiently extract a valid witness $w$. Formally, for any prover $\Prover^*$:
