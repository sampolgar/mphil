...
\subsection{BBS+ \cite{hutchison_constant-size_2006}}
...
\begin{itemize}
    \item \textbf{Precomputed Values}
        \begin{itemize}
            \item $e(g_1, \hat{h_0}), e(g_2, \hat{h_0}), e(g_2, \hat{w})$ \qquad Stored constants in $\G_T$
            \item $e(g_{i+1}, \hat{h_0})$ for $i \in [1,L]$ \qquad L stored constants in $\G_T$
        \end{itemize}
    \item \textbf{Prover's Initial Operations}
        \begin{itemize}
            \item Set $\delta_1 = r_1e, \delta_2 = r_2e$ \qquad $2F_p\times$
            \item $A_1 = g_1^{r_1}g_2^{r_2}$ \qquad $2E_{\G_1} + 1M_{\G_1}$
            \item $A_1^e$ computation \qquad $1E_{\G_1}$
            \item $A_2 = Ag_2^{r_1}$ \qquad $1E_{\G_1} + 1M_{\G_1}$
        \end{itemize}
    \item \textbf{Prover Schnorr Operations}
    \begin{itemize}
        \item Sample L+6 field elements (negligible)
        \item $T_1 = g_1^{\tilde{r_1}}g_2^{\tilde{r_2}}$ \qquad $2E_{\G_1} + 1M_{\G_1}$
        \item $T_2 = g_1^{\tilde{\delta_1}}g_2^{\tilde{\delta_2}}$ \qquad $2E_{\G_1} + 1M_{\G_1}$
        \item $T_3$ computation: \qquad $1P + (L+4)E_{\G_T} + (L+3)M_{\G_T}$
            \begin{itemize}
                \item $e(A_2, \hat{h_0})^{\tilde{-e}}$ \qquad $1P + 1E_{\G_T}$
                \item Remaining exponentiations use precomputed pairings \qquad $(L+3)E_{\G_T} + (L+3)M_{\G_T}$
            \end{itemize}
    \end{itemize}
        \item \textbf{Proof Size / Data Sent}
            \begin{itemize}
                \item $(A_1, A_1^e, A_2, T_1, T_2, T_3)$ \qquad $5E_{\G_1} + 1E_{\G_T}$
                \item Responses $z_x$ for $x \in {r_1, r_2, e, \delta_1, \delta_2, s, m_1,\ldots,m_L}$ \qquad $(L+6)\Z_p^*$
            \end{itemize}
    \item \textbf{Verifier Operations}
        \begin{itemize}
            \item First equation: $A_1^c \cdot T_1 \stackrel{?}{=} g_1^{z_{r_1}}g_2^{z_{r_2}}$ \qquad $3E_{\G_1} + 2M_{\G_1}$
            \item Second equation: $(A_1^e)^c \cdot T_2 \stackrel{?}{=} g_1^{z_{\delta_1}}g_2^{z_{\delta_2}}$ \qquad $3E_{\G_1} + 2M_{\G_1}$
            \item Final pairing equation: \qquad $2P + (L+4)E_{\G_T} + (L+3)M_{\G_T}$
                \begin{itemize}
                    \item Left side: $\frac{e(A_2, \widehat{w})}{e(g_2, \widehat{h_0})}$ \qquad $2P$ + $1M_{\G_T}$
                    \item Right side  \qquad $(L+3)E_{\G_T} + (L+2)M_{\G_T}$
                \end{itemize}
        \end{itemize}
\end{itemize}
...
\newpage
\subsection{BBS+ \cite{camenisch_anonymous_2016}}
asd
\subsubsection{Protocol for Signature Proof of Knowledge}
asd
...
\begin{itemize}
    \item \textbf{Notation:}
        \begin{itemize}
            ...
            \item $E_{\G_1, \G_2, \G_T}$, $M_{\G_1, \G_2, \G_T}$: Exponentiation / Multiplication in respective groups
            \item $P$: Pairing Operation
            \item $n$: Number of attributes/messages
        \end{itemize}
    \item \textbf{Precomputed Values}
        \begin{itemize}
            \item $e(g, g_2), e(h_0, g_2)$ \qquad Stored constants in $\G_T$
            \item $e(h_i, g_2)$ for $i \in [1,n]$ \qquad n stored constants in $\G_T$
        \end{itemize}
    \item \textbf{Prover's Operations}
        \begin{itemize}
            \item $r_3 = \frac{1}{r_1}$ \qquad $1F_p-$
            \item $s' = s - r_2 \cdot r_3$ \qquad $1\F_p\times + 1F_p-$
            \item $A' \gets A^{r_1}$ \qquad $1E_{\G_1}$
            \item $\Bar{A} \gets A'^{-e} \cdot b^{r_1}$ \qquad $2E_{\G_1} + 1M_{\G_1}$
            \item $d \leftarrow (gh_0^{r_1}\prod h_i^{-m_i})^{r_2} \cdot h_0^{r_3}$
                \begin{itemize}
                    \item $h_0^{r_1}$ \qquad $1E_{\G_1}$
                    \item $\prod h_i^{-m_i}$ \qquad $nE_{\G_1} + (n-1)M_{\G_1}$
                    \item Final exponentiation and multiplication \qquad $1E_{\G_1} + 2M_{\G_1}$
                \end{itemize}
        \end{itemize}
    \item \textbf{Prover Schnorr Operations}
        \begin{itemize}
            \item Sample n+4 field elements (negligible)
            \item $T_1 \gets A^{\tilde{e}} \cdot h_0^{\tilde{s}}$ \qquad $2E_{\G_1} + 1M_{\G_1}$
            \item $T_2 \gets g\prod_{i=1}^n h_i^{\tilde{m_i}}$ \qquad  $nE\G_1 + (n-1)M_{\G_1}$
            \item Compute n+4 responses (negligible)
        \end{itemize}
    \item \textbf{Proof Size / Data Sent}
        \begin{itemize}
            \item $(A', \Bar{A}, d, T_1, T_2)$ \qquad $5\G_1$ elements
            \item Responses for ${r_1, r_2, e, s', m_1, \ldots, m_n}$ \qquad $(n + 4)\Z_p^*$
        \end{itemize}
    \item \textbf{Verifier Operations}
        \begin{itemize}
           \item Pairing equality check: $e(A',\hat{w}) = e(\Bar{A},g_2)$ \qquad $2P + 1M_{\G_T}$
        \end{itemize}
    \item \textbf{Verifier Schnorr Operations}
        \begin{itemize}
            \item $T_1 \stackrel{?}{=} A^{z_e} \cdot h_0^{z_s} \cdot (\Bar{A})^{-c}$ \qquad $3E_{\G_1} + 2M_{\G_1}$
            \item $T_2 \stackrel{?}{=} g^c\prod_{i=1}^n h_i^{z_{m_i}}$ \qquad $(n+1)E_{\G_1} + nM_{\G_1}$
        \end{itemize}
\end{itemize}
...
Signature scheme D from \cite{hutchison_signature_2004}
% \begin{itemize}
%     \item \textbf{KeyGen}($1^\lambda$):
%     \begin{itemize}
%         \item $\mathsf{BG} = (p, \G_1, \G_2, \G_T, e, g, \hat{g}) \sample \mathsf{BGGen}(1^\lambda)$
%         \item Sample secret key components: $x, y \sample \Z_p^*$, $(z_1, \ldots, z_\ell) \sample (\Z_p^*)^\ell$. Set $sk = (x, y, z_1, \ldots, z_\ell)$
%         \item Compute public key components: Compute public $x,y$ by $X \gets g^x \in \G_1$, $\widehat{Y} \gets \hat{g}^y \in \G_2$
%         For $i \in [1..\ell]$: $\widehat{Z}_i \gets \hat{g}^{z_i} \in \G_2$, $W_i \gets X^{z_i} \in \G_1$
%         \item Output: $pk = (\mathsf{BG}, X, \widehat{Y}, \{\widehat{Z}_i\}_{i=1}^\ell, \{W_i\}_{i=1}^\ell)$
%     \end{itemize}
    
%     \item \textbf{Sign}($sk = (x, y, z_1, \ldots, z_\ell), (m_1,\ldots,m_\ell)$):
%     \begin{itemize}
%         \item Sample $\alpha \sample \Z_p^*$. Let $a \gets g^\alpha \in \G_1$
%         \item For $i \in [1..\ell]$: Compute $A_i \gets a^{z_i} \in \G_1$. Set $b \gets a^y \in \G_1$, $B_i \gets A_i^y \in \G_1$
%         \item Compute $c \gets a^{x+\alpha xy + \sum_{i=1}^\ell m_i xy z_i} \in \G_1$
%         \item Output $\sigma = (a, \{A_i\}_{i=1}^\ell, b, \{B_i\}_{i=1}^\ell, c)$
%     \end{itemize}

%     \item \textbf{Verify}($pk = (\mathsf{BG}, X, \widehat{Y}, \{\widehat{Z}_i\}_{i=1}^\ell, \{W_i\}_{i=1}^\ell), (m_1,\ldots,m_\ell), \sigma$):
%     \begin{itemize}
%         \item Parse $\sigma = (a, \{A_i\}_{i=1}^\ell, b, \{B_i\}_{i=1}^\ell, c)$
%         \item Check signature structure:
%             \begin{itemize}
%                 \item $e(b,\hat{g}) = e(a,\widehat{Y})$
%                 \item $\forall i \in [1..\ell]: e(B_i,\hat{g}) = e(A_i,\widehat{Y})$
%                 \item $e(c,\hat{g}) = e(X,\hat{g}) \cdot e(a,\widehat{Y})^\alpha \cdot \prod_{i=1}^\ell e(A_i,\widehat{Y})^{m_i}$
%             \end{itemize}
%         \item Output accept if all checks pass, reject otherwise
%     \end{itemize}
% \end{itemize}
% \begin{itemize}
%     \item \textbf{BlindSign}:
%         \begin{itemize}
%             \item \textbf{Common Input}: $pk$, commitment $M \in \G_1$
%             \item \textbf{User Input}: Messages $(m_1,\ldots,m_\ell)$ such that $M = g^{m_1}\prod_{i=1}^\ell \widehat{Z}_i^{m_i}$
%             \item \textbf{Signer Input}: $sk$
%             \item \textbf{Protocol}:
%             \begin{itemize}
%                 \item User $\to$ Signer: Proof $\pi = \mathsf{PoK}\{(m_1,\ldots,m_\ell): M = g^{m_1}\prod_{i=1}^\ell \widehat{Z}_i^{m_i}\}$
%                 \item Signer: Verify $\pi$, then:
%                     \begin{itemize}
%                         \item Sample $\alpha \sample \Z_p^*$, compute $a \gets g^\alpha \in \G_1$
%                         \item For $i \in [1..\ell]$: $A_i \gets a^{z_i} \in \G_1$
%                         \item Set $b \gets a^y \in \G_1$, $B_i \gets A_i^y \in \G_1$
%                         \item Compute $c \gets a^x \cdot M^{xy} \in \G_1$
%                     \end{itemize}
%                 \item Signer $\to$ User: $\sigma = (a, \{A_i\}_{i=1}^\ell, b, \{B_i\}_{i=1}^\ell, c)$
%             \end{itemize}
%         \end{itemize}
%     \item \textbf{ZKPoK-Verify}:
%         \begin{itemize}
%             \item \textbf{Common Input}: $pk$, blinded signature $\tilde{\sigma} = (\tilde{a}, \{\tilde{A}_i\}, \tilde{b}, \{\tilde{B}_i\}, \tilde{c})$
%             \item \textbf{Prover Input}: $(m_1,\ldots,m_\ell)$, randomization factors $r, r'$
%             \item \textbf{Protocol}:
%             \begin{itemize}
%                 \item Compute pairings:
%                     \begin{itemize}
%                         \item $v_x \gets e(\tilde{a},\hat{g})$
%                         \item $v_{xy} \gets e(\tilde{a},\widehat{Y})$
%                         \item $v_s \gets e(\tilde{c},\hat{g})$
%                     \end{itemize}
%                 \item Execute proof: $\pi = \mathsf{PoK}\{(r,r',\{m_i\}): v_s = v_x \cdot v_{xy}^r \cdot \prod_{i=1}^\ell e(\tilde{A}_i,\widehat{Y})^{m_i}\}$
%                 \item Verify: $e(\tilde{b},\hat{g}) = e(\tilde{a},\widehat{Y})$ and $\forall i: e(\tilde{B}_i,\hat{g}) = e(\tilde{A}_i,\widehat{Y})$
%             \end{itemize}
%         \end{itemize}
%         \item \textbf{ZKPoK-Verify}:
%             \begin{itemize}
%                 \item \textbf{Common Input}: $pk$, signature $\sigma = (a, \{A_i\}_{i=1}^\ell, b, \{B_i\}_{i=1}^\ell, c)$
%                 \item \textbf{Prover Input}: $(m_1,\ldots,m_\ell)$
%                 \item Send $(\tilde{a}, \{\tilde{A}_i\}, \tilde{b}, \{\tilde{B}_i\}, \tilde{c})$ to Verifier

%                 \item \textbf{Protocol}:
%                 \begin{itemize}
%                     \item Sample $r, r' \sample \Z_p^*$, Compute $(a^{r'}, b^{r'}, c^{r'r}) = (\tilde{a}, \tilde{b}, \tilde{c^r})$
%                     \item For $i \in [1..\ell]$: $\tilde{A}_i \gets A_i^r$, and $\tilde{B}_i \gets B_i^r$
%                     \item Send $(\tilde{a}, \{\tilde{A}_i\}, \tilde{b}, \{\tilde{B}_i\}, \tilde{c^r})$ to Verifier
%                     \item Both parties compute:
%                     \[
%                     v_x \gets e(\tilde{a},\hat{g}) \qquad v_{xy} \gets e(\tilde{a},\widehat{Y}) \qquad v_s \gets e(\tilde{c^r},\hat{g})
%                     \]
%                     \item Execute proof: 
%                         \[\pi = \mathsf{PoK}\{(r,r',\{m_i\}): v_s = v_x \cdot v_{xy}^r \cdot \prod_{i=1}^\ell e(\tilde{A}_i,\widehat{Y})^{m_i}\}\]
%                     \item Verifier checks:
%                         \begin{itemize}
%                             \item $e(\tilde{b},\hat{g}) = e(\tilde{a},\widehat{Y})$
%                             \item $\forall i \in [1..\ell]: e(\tilde{B}_i,\hat{g}) = e(\tilde{A}_i,\widehat{Y})$
%                             \item Verification of $\pi$
%                         \end{itemize}
%                 \end{itemize}
% \end{itemize}
% \end{itemize}
...
\newpage
\subsection{CL04 Mapped to Type3 Pairings}
\begin{itemize}
    \item \textbf{KeyGen}($1^\lambda$):
    \begin{itemize}
        \item $\mathsf{BG} = (p, \G_1, \G_2, \G_T, e, g, \hat{g}) \sample \mathsf{BGGen}(1^\lambda)$
