...
Unforgeability
Single-credential: Standard EUF-CMA security from your underlying signature scheme
Multi-credential: Credential binding and cross-credential consistency
- no user can present credentials with different identities
- cannot mix and match attributes across credentials
- cannot reuse credentials across contexts
...
% \begin{theorem}[Master Credential Type 1 Unforgeability]
% If the rerandomizable signature scheme RS is EUF-CMA secure, then no PPT adversary can win 
% the MIMC-ABC unforgeability game with Type 1 forgery with non-negligible probability.
% \end{theorem}

% \begin{proof}
% We construct an adversary B against the EUF-CMA security of RS using any adversary A that 
% produces a Type 1 forgery. B operates as follows:

% \paragraph{Setup.} Upon receiving EUF-CMA challenge $(pk^* = (vk^*, ck^*))$:
% \begin{itemize}
%     \item Set master issuer key $opk_m = (vk^*, ck^*)$
%     \item Initialize $HU, CU, CRED \leftarrow \emptyset$
%     \item Give $opk_m$ to A
% \end{itemize}

% \paragraph{Oracle Simulation.} B simulates the following oracles:

% $\mathcal{O}_{HU}(i)$:
% \begin{itemize}
%     \item Sample $usk_i \gets \mathbb{Z}_p$
%     \item Add $i$ to $HU$
%     \item Return $usk_i$
% \end{itemize}
...
% $\mathcal{O}_{CU}(i)$:
% \begin{itemize}
%     \item If $i \in HU \setminus CU$:
%         \begin{itemize}
%             \item Add $i$ to $CU$
%             \item Return all credentials and secrets for user $i$
%         \end{itemize}
%     \item Return $\bot$
% \end{itemize}

% $\mathcal{O}_{ObtainMaster}(i, \vec{m})$:
% \begin{itemize}
%     \item If $i \notin HU$ return $\bot$
%     \item Compute $cm = \mathsf{CM.Com}([\vec{m}]; usk_i)$
%     \item Receive proof $\pi$ from $\mathcal{A}$ proving knowledge of opening
%     \item Run knowledge extractor $\mathcal{E}$ on $\pi$ to get $(\vec{m}', usk')$
%     \item Verify $cm = \mathsf{CM.Com}([\vec{m}']; usk')$
%     \item If verification fails: return $\bot$
%     \item Query EUF-CMA signing oracle on $cm$ to get $\sigma$
%     \item Store $(cm, \sigma, \vec{m}', usk')$ in $CRED$
%     \item Return $(cm, \sigma)$
% \end{itemize}

% $\mathcal{O}_{Show}$ operates normally using stored credentials.

% \paragraph{Forgery Extraction.} When A outputs $(π^*, cred^* = (cm^*, σ^*))$:

% \begin{itemize}
%     \item Run knowledge extractor $\mathcal{E}$ on $π^*$ to obtain $(\vec{m}^*, usk^*)$
%     \item Verify commitment: $cm^* = \mathsf{CM.Com}([\vec{m}^*]; usk^*)$
%     \item Verify signature: $e(σ_2, \tilde{g}) = e(σ_1, vk^* \cdot \widehat{cm^*})$
%     \item If either verification fails: abort
% \end{itemize}

% B distinguishes between two forgery cases:

% \paragraph{Case 1 (New Commitment).} If $cm^* \notin CRED$:
% \begin{itemize}
%     \item $(cm^*, σ^*)$ is a valid EUF-CMA forgery because:
%         \begin{itemize}
%             \item $σ^*$ verifies under $vk^*$
%             \item $cm^*$ was never queried to signing oracle
%         \end{itemize}
%     \item Output $(cm^*, σ^*)$ to break EUF-CMA
% \end{itemize}

% \paragraph{Case 2 (Credential Theft).} If $\exists(cm, σ, \vec{m}, usk) \in CRED$ where:
% \begin{itemize}
%     \item $cm = cm^*$
%     \item Owner $i \in HU$ (honest user)
%     Then:
%     \item $(cm^*, σ^*)$ is a valid EUF-CMA forgery because:
%         \begin{itemize}
%             \item The commitment was signed for honest user $i$
%             \item $\mathcal{A}$ could not have known $usk_i$ to create valid proof
%         \end{itemize}
%     \item Output $(cm^*, σ^*)$ to break EUF-CMA
% \end{itemize}

% \paragraph{Analysis.} If $\mathcal{A}$ succeeds with non-negligible probability $ε$, then B breaks 
% EUF-CMA security of RS with probability at least $ε - negl(λ)$ where $negl(λ)$ accounts for:
% \begin{itemize}
%     \item Knowledge extraction failure probability
%     \item Commitment scheme binding failure probability
% \end{itemize}

% The reduction succeeds because any Type 1 forgery must either be a new commitment (Case 1) or 
% reuse a commitment from an honest user (Case 2). In either case, B produces a valid EUF-CMA 
% forgery against the PS signature scheme.
% \end{proof}
...
% What changes in the mimc scenario?

% Unforgeability: 


% Forgery scenarios
% - combining attributes from different issuers maliciously
% - context credential decoupling: use context credential without a valid master credential
% -- mitigation: enforce credential chains (enforce all credentials are valid + equal is true)
...
\subsection{Master Credential Type-1 Forgery}
\begin{theorem}
    If the signature scheme $\RS$ is EUF-CCA secure and the commitment scheme $\CM$ is binding, then no $\PPT$ adversary $AdvA$ can forge a master credential with non-negligible probability.
\end{theorem}

\subsubsection{Reduction}
\begin{itemize}
    \item \textbf{Setup: } Receive EUF-CCA challenger's public key $pk*$, set as master issuer public key $opkm$. Maintain lists CRED, COM, REL (relation between creds and users)
    \item \textbf{Oracle Simulation: } B simulates A's environment
    \item \textbf{Forgery Extraction: }
\end{itemize}
