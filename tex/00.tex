...
\begin{theorem}[Single-Issuer Multi-Credential Unforgeability]
If the rerandomizable signature scheme RS is EUF-CMA secure, the commitment scheme RVC is binding, and the zero-knowledge proof system is sound, then the single-issuer multi-credential anonymous credential system satisfies unforgeability according to $\mathrm{Exp}^{\mathsf{UNF}}_{\mathsf{SingleIssuer\text{-}MultiCred}, \adv}(\lambda)$.
\end{theorem}
...
\begin{pchstack}[boxed, center, space=1em]
    \procedure[]{$\mathrm{Exp}^{\mathsf{UNF}}_{\mathsf{MultiIssuer\text{-}MultiCred}, \adv}(\lambda)$}{%
        \pccomment{Challenger Setup} \\
        \ppar \gets \Setup(\secparam) \\
        \ck \gets \CMSetup(\secparam) \\
        \text{For each issuer } j \in [n]: (\osk_j, \opk_j) \gets \OrgKeyGen(\ppar) \\
        \text{Initialize } \HU \gets \emptyset, \; \CU \gets \emptyset \\
        \text{For each issuer } j \in [n]: \CRED_j \gets \emptyset, \; \OWNR_j \gets \{\} \\
        \\
        \pccomment{Adversary Interaction} \\
        \AdvA^{\OHU, \OCU, \OOBTMASTER, \OOBTCONTEXT, \OSHOW}(\{\opk_j\}_{j \in [n]}, \ck) \\
        \\
        \pccomment{Adversary outputs forgery} \\
        \AdvA \text{ outputs } (\{\cred_k'^*\}_{k \in K}, \{\cm_k'^*\}_{k \in K}, \pi^*, \phi^*) \\
        \\
        \pccomment{Winning Condition} \\
        \MIMCVerify(\{\cred_k'^*\}_{k \in K}, \{\cm_k'^*\}_{k \in K}, \pi^*, \phi^*) = 1 \; \wedge\\
        \nexists\, i \in \CU \text{ such that } \forall k \in K: \\
        \t (\id_i, \attrs_k, i, \cdot, \cdot) \in \CRED_{j_k} \; \wedge \; \phi^*(\{(\id_i, \attrs_k)\}_{k \in K}) = 1\\
    }
    \begin{pcvstack}
        \procedure[]{$\OHU(i)$}{%
            \pcif i \notin \HU \cup \CU \\ 
            ...
            \t \id_i \sample \{0,1\}^\lambda \pccomment{Unique identifier} \\
            \pcreturn i \\
        }
        ...
        \procedure[]{$\OCU(i)$}{%
            ...
            \pcreturn \usk[i], \id_i, \\
            \t \bigcup_{j \in [n]} \{(\cred, \cm, \attrs) \in \CRED_j \mid \OWNR_j[\cred] = i\} \\
        }
        
        \procedure[]{$\OOBTMASTER(i, \attrsm, j)$}{%
            \pcif i \notin \HU, \pcreturn \bot \\
            \t \cmm \gets \CMCom([\id_i, \attrsm]; \usk[i]) \\
            \t \sigmam \gets \Issue_j(\osk_j, \cmm) \\
            \t \credm \gets (\sigmam, \cmm, \opk_j) \\
            \t \CRED_j \gets \CRED_j \cup \{(\id_i, \attrsm, i, \cmm, \sigmam)\} \\
            \t \OWNR_j[\credm] \gets i \\
            \pcreturn \credm \\
        }
        
        \procedure[]{$\OOBTCONTEXT(i, \attrsc, \credm, j)$}{%
            \pcif i \notin \HU, \pcreturn \bot \\
            \t \pccomment{Verify master credential} \\
            \t \text{Parse } \credm \text{ as } (\sigmam, \cmm, \opk_{j'}) \\
            \t \text{Find } j' \text{ such that } (\id_i, \attrsm, i, \cmm, \sigmam) \in \CRED_{j'} \\
            \t \pcif \text{no such credential exists}, \pcreturn \bot \\
            \t \pccomment{Create context credential} \\
            \t \cmc \gets \CMCom([\id_i, \attrsc]; \usk[i]) \\
            \t \pccomment{Generate equality proof for identities} \\
            \t \pi_{\text{eq}} \gets \ZKProve(\cmm, \cmc, \id_i, \usk[i]) \\
            \t \sigmac \gets \Issue_j(\osk_j, \cmc, \pi_{\text{eq}}) \\
            \t \credc \gets (\sigmac, \cmc, \opk_j) \\
            \t \CRED_j \gets \CRED_j \cup \{(\id_i, \attrsc, i, \cmc, \sigmac)\} \\
            \t \OWNR_j[\credc] \gets i \\
            \pcreturn \credc \\
        }
        
        \procedure[]{$\OSHOW(i, \phi, \{j_k\}_{k \in K})$}{%
            \pcif i \notin \HU, \pcreturn \bot \\
            \t \pccomment{Find credentials from issuers in $\{j_k\}$ that satisfy $\phi$} \\
            \t \text{For each } k \in K, \text{ find } (\id_i, \attrs_k, i, \cm_k, \sigma_k) \in \CRED_{j_k} \\
            \t \pcif \text{missing any required credential}, \pcreturn \bot \\
            \t \pcif \phi(\{(\id_i, \attrs_k)\}_{k \in K}) = 0, \pcreturn \bot \\
            \t \pccomment{Rerandomize credentials} \\
            \t \text{For each } k \in K: \\
            \t \t (\cred_k', \cm_k', \pi_k) \gets \Show(\sigma_k, \cm_k, \usk[i]) \\
            \t \pccomment{Generate cross-credential proof} \\
            \t \pi \gets \ZKProve(\{\cm_k'\}_{k \in K}, \id_i, \{\attrs_k\}_{k \in K}, \phi) \\
            \pcreturn (\{\cred_k'\}_{k \in K}, \{\cm_k'\}_{k \in K}, \pi) \\
        }
    \end{pcvstack}
\end{pchstack}
