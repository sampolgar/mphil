...
\begin{itemize}
    ...
    \item $\mathsf{CM.Rerand}(\ck, \cm, \widehat{\cm}, r_\Delta) \to (\mathsf{cm'}, \widehat{\mathsf{cm'}}):$
    On input $\ck = (\mathsf{BG}, \{G_i, \widehat{G}_i\}_1^n, H,\widehat{H})$, commitments $\cm, \widehat{cm}$ and new randomness $r_\Delta$, compute blinding points $r_\DeltaH, r_\Delta\widehat{H}$, add points to commitments, $\cm' = \cm + r_\DeltaH$, and $\widehat{\cm'} = \widehat{\cm} + r_\Delta\widehat{H}$, the randomized commitment is now $\cm = m_1G_1 + \ldots + m_iG_i + (r + r_\Delta)H$ and new opening key $(r + r_\Delta)$
\end{itemize}
...
\begin{itemize}
    \item \textbf{Rerandomization: } given a commitment $\cm = mG + rH$ and fresh randomness $r_\Delta \in \Z_p$, the rerandomized commitment $\cm'$ is $\cm + r_\DeltaH$ = $mG + rH + r_\DeltaH$ = $mG + (r+r_\Delta)H$. This operation preserves the committed value $m$ while updating the randomness from $r$ to $r + r_\Delta$. $\cm'$ is computationally indistinguishable from $\cm$.
...
\begin{definition}[Rerandomizable Signature over Commitments]
    A rerandomizable signature scheme over commitments $\mathsf{RS}$ is a tuple $(\mathsf{KeyGen}, \mathsf{Sign}, \mathsf{Rerand}, \mathsf{Ver}, \mathsf{VerKey})$ of PPT algorithms where:
        \begin{itemize}
            ...
            \item $\mathsf{RS.Rerand}(\pk, \sigma, r_\Delta, u_\Delta) \rightarrow \sigma'$ is a deterministic algorithm that enables signature rerandomization. Takes a public key $\pk = (\mathsf{ck,vk})$, a signature $\sigma$, and randomization elements $r_\Delta, u_\Delta$, as input, outputs a new signature $\sigma'$. For any message $m$ and initial randomizer $r$, if $\sigma$ was generated as $\sigma \gets \mathsf{RS.Sign}(\sk, \mathsf{CM.Com}(\ck, m, r), u)$, then the output $\sigma'$ corresponds to $\mathsf{RS.Sign}(\sk,\mathsf{CM.Com}(\ck, m, r+r_\Delta), u+u_\Delta)$. The randomness spaces for both the signature and commitment schemes form algebraic groups under the efficiently computable operation $"+"$.
...
\begin{itemize}
    ...
    \item $PS.Sign_{\sk}(\cm) \to \sigma: $ The signing algorithm signs the commitment. Retreives $(g, \cdot, \tilg, \cdot)$ from $\ck$, Samples $u \sample \Zp$, Computes $\sigma_1$ as $g^u$ and $\sigma_2 \gets (\sk \cdot \cm)^u$, returns $(g^u, (\sk \cdot \cm)^u)$

    \item $PS.Rerand(\sigma, \rd, \ud) \to \sigma':$ Parse $\sigma$ as $(\sigma_1, \sigma_2) \equiv (h, \sigma_2) \equiv (g^u, (\sk \cdot \cm)^u)$. Compute $\sigma_1' \gets \sigma_1^{\ud}$ and $\sigma_2' \gets (\sigma_2 \cdot \sigma_1^{\rd})^{\ud}$. Return $\sigma' \gets (\sigma_1', \sigma_2')$
