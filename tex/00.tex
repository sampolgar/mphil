...
\begin{corollary}[Proving Zero Values]
    
\end{corollary}

\begin{corollary}[Equality of messages within commitments]
    
\end{corollary}

% Corollary 1: To prove that mᵢ = 0 for some index i, run Construction 2 with modified 
% commitment key ck' = (g, g₁,...,gᵢ₋₁,gᵢ₊₁,...,gₗ) omitting gᵢ.


% Corollary 1.1: Proving Zero Values:
% By omitting generators gᵢ from the commitment key, Construction 1 
% proves that corresponding mᵢ = 0...


% Corollary 1.2: Multi-Message Application
% [If needed, specific details about multi-message case]



% For commitments cm₁ = g₁ᵐh₁ʳ¹ and cm₂ = g₂ᵐh₂ʳ², prove knowledge of 
% (m,r₁,r₂) by running Construction 1 with linked responses:
% - Use same α for g₁ᵐ and g₂ᵐ terms
% - Different ρ₁,ρ₂ for h₁ʳ¹,h₂ʳ² terms
% - Response s used in both verification equations
% This proves m values are equal without revealing m.
...
\begin{construction}{Proof of Zero($C$)}{zeroproof}
    \vspace{1em}
    \textbf{Public Parameters:} Commitment key $ck = g_1, \dots, g_i, h\in \G$. 
    
    For proof of zeros, $ck' = ck$ with a specific generator $g_i$ removed
    ...
    \vspace{1em}
        ...
        \begin{enumerate}
        \item $\Prover$ samples $\alpha_1, \dots, \alpha_i, \rho \sample [q-1]$ and sends $T \gets \prod_{i=1}^n g_i^{\alpha_i}h^{\rho}$
