...
Traditional unforgeability definitions are insufficient for anonymous credential systems because rerandomizable credentials and zero-knowledge proofs enable legitimate reuse and selective disclosure that would be classified as forgeries under EUF-CMA security. Our definition addresses two critical attack vectors:
\begin{enumerate}
...
\begin{proof}
We construct a reduction from the unforgeability of the multi-credential system to the $\EUFCMA$ security of the underlying signature scheme. Given an adversary $\adv$ that breaks credential unforgeability with non-negligible probability $\epsilon$, we build an adversary $\advb$ that breaks $\EUFCMA$ with related probability.

\textbf{Simulator Setup:} $\advb$ receives verification key $\vk$ from the $\EUFCMA$ challenger and access to a signing oracle $\OSIGN(\cdot)$. It sets $\opk = \vk$ and simulates the credential system experiment.

\textbf{Oracle Simulation:}
\begin{itemize}
    \item $\OHU$ and $\OCU$: Implemented as in the experiment definition.
    
    \item $\OOBTMASTER(i, \attrsm)$: Compute $\cmm = \CMCom([id_i, \attrsm]; \usk[i])$, obtain $\sigmam \gets \OSIGN(\cmm)$, record in $\CRED_M$, return $(\sigmam, \cmm)$.
    
    \item $\OOBTCONTEXT(i, \attrsc, \credm)$: Verify $\credm$ exists in $\CRED_M$ for user $i$, compute $\cmc = \CMCom([id_i, \attrsc]; \usk[i])$, verify binding proof, obtain $\sigmac \gets \OSIGN(\cmc)$, record in $\CRED_C$, return $(\sigmac, \cmc)$.
    
    \item $\OSHOW$: Implemented as specified, generating presentations for valid credential pairs.
\end{itemize}

\textbf{Forgery Classification and Analysis:} When $\adv$ outputs $(\credm'^*, \credc'^*, \cmm'^*, \cmc'^*, \pi^*, \phi^*)$ satisfying the winning condition, we must establish which type of forgery has occurred and extract a valid $\EUFCMA$ forgery.

\textbf{Case Analysis:} We consider three exhaustive cases:

\textbf{Case 1:} At least one of $\cmm'^*$ or $\cmc'^*$ is not a rerandomization of any commitment in $\CRED_M$ or $\CRED_C$ respectively.

\textbf{Case 2:} Both $\cmm'^*$ and $\cmc'^*$ are rerandomizations of commitments in $\CRED_M$ and $\CRED_C$, but they come from different users.

\textbf{Case 3:} Both $\cmm'^*$ and $\cmc'^*$ are rerandomizations of commitments belonging to the same corrupt user, but the attributes don't satisfy $\phi^*$.
\end{itemize}

\textbf{Extraction of $\EUFCMA$ Forgery:} 
For Case 1, we directly obtain a signature forgery. 
For Cases 2 and 3, we rely on the soundness of the zero-knowledge proof system to establish a contradiction with our winning condition.
\end{proof}
