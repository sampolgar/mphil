...
\begin{itemize}
    ...
    \item $\mathsf{VRF.Eval}(sk, x) \to y, \pi:$ Returns output $y \gets g^{1/(x+sk)}$ and $\pi$ verifies the output $y$
    
    \item $\mathsf{VRF.Vfy}(pk, x, y, \pi) \to \bit:$ validates proof $\pi$ that $y = g^{1/(x+sk)}$, outputs 1 for success, 0 for failure
\end{itemize}
\end{definition}
...
\subsection{Construction}
We instantiate our VRF with a prime-order group $\G$ and generator $g$, we assume the existence of two pedersen commitments, with at a minimum, key $k$ in one, and $\textsf{ctx}$ in the other, for example, $\cm_k = g^k h^{r_1}$, $\cm_{\textsf{ctx}} = g^{\textsf{ctx}} h^{r_2}$. In practice, the commitments may may have a different structure and more committed messages but require this basic structure. 

\noindent Let $\G$ be a prime-order group with generator $g$, our VRF scheme consists of
\begin{itemize}
    \item $\mathsf{VRF.Eval}(k, \textsf{ctx}) \to (\mathsf{N}, \Pi^{\mathcal{R}_{\textsf{VRF}}})$: Compute $m_1 = k + \text{ctx} \bmod q$, $m_2 = 1/m_1$. Compute nullifier $\textsf{N} = g^{m_2}$ and run generate $\Pi^{\mathcal{R}_{\textsf{VRF}}}$ from 
    \[
    ...
    \item $\mathsf{VRF.Vfy}(\cm_k, \cm_{\textsf{ctx}}, \mathsf{N}, \Pi^{\mathcal{R}_{\textsf{VRF}}})$
...
\subsubsection{VRF Proof generalises to Proof of Multiplicative Inverse}
...
% \begin{lemma}[Equivalence to Standard VRF]
% Under the q-DHI assumption, there exists PPT simulator $\mathcal{S}$ such that:
% \[
% \forall x, (\text{N},\pi) \approx_c (\mathsf{Eval}(sk,x), \mathsf{Prove}(sk,x))
% \]
...
Within our anonymous credential system, a user has a Master Credential with a PRF key $k$ and Context Credential with $\textsf{ctx}$, where $\textsf{ctx}$ is the context, such as $\mathcal{H}(\textit{"DriversLicense"})$ where $\mathcal{H}$ is hashes a string to $\Z_p$
\[
...
\subsection{Computing VRF on Committed Inputs}
...
\subsection{From Pairings to Multiplicative Inverses}
The classical Dodis-Yampolskiy VRF uses pairings to verify the relationship between inputs and outputs through the equation:
\[
e(g_1^{\textsf{ctx}} \cdot pk, \mathsf{N}) = e(g_1, g_2)
\]

Our key insight is that this pairing equation fundamentally verifies a multiplicative relationship. When we expand the left side:
\[
e(g_1^{\textsf{ctx}} \cdot g_1^k, g_2^{1/(k + \textsf{ctx})}) = e(g_1, g_2)
\]
we are effectively proving that $(k + \textsf{ctx}) \cdot \frac{1}{k + \textsf{ctx}} = 1$. 
...
\begin{itemize}
    ...
    \item Derived commitment to their sum:
        \[\cm_3 = g^{m_1} h^{r_3} \text{ where } m_1 = k + \textsf{ctx}\]
    
    \item Commitment to the inverse:
        \[\cm_4 = g^{m_2} h^{r_4} \text{ where } m_2 = \frac{1}{m_1}\]
\end{itemize}
...
Given these commitments, we construct a $\Sigma$-protocol that proves the VRF relationship in zero-knowledge. The protocol leverages auxiliary commitments $\cm_3, \cm_4$ to enforce the multiplicative inverse relationship:
