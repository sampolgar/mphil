...
combining three cryptographic primitives in a novel way that preserves privacy 
\begin{enumerate}
    \item \textbf{Modified Rerandomizable Pointcheval-Sanders Signatures:} 
    We employ a modified PS Signature scheme optimized for anonymous credentials. Our construction provides constant-size signatures and verification cost via a pairing-based construction. Standard PS Signature rerandomization changes the underlying Pedersen commitment structure, which requires customized proofs of knowledge as they must simultaneously prove knowledge of a signature and attributes. We use a trick from \cite{tomescu2022utt} to enable simultaneous randomization of the signature and underlying commitment; the key is that signature verification yields a verified Pedersen commitment to use directly in standardized zero-knowledge proof protocols rather than bespoke versions with questionable security. Furthermore, this simplifies compound proof statements about multiple credentials as standard sigma protocols are well-studied. 


    \item \textbf{Commitment Scheme: } We leverage pedersen commitments for their properties: perfect hiding, computational binding, position binding, and homomorphism. The binding property coupled with proof soundness is used to prove unforgeability, the hiding property maintains zero knowledge to prove anonymity. Position binding enables efficient equality proofs across different credentials which is used to prove multiple independently issued credentials belong to the same user. Homomorphic properties enable complex interactive protocols where commitments are combined to prove certain relations hold. We use dual pedersen commitments as in \cite{tomescu2022utt} to commit to attributes in different groups to be used in conjunction with the pairing-based PS signature. 

    \item \textbf{Zero-Knowledge Proofs} 
    Our construction liberally employs Sigma protocols for proving statements about committed attributes. While proofs scale linearly with attribute size and number of credentials to verify, Sigma protocols provide the most efficient proof mechanism in practice, making the overhead negligible. The combination of Sigma protocols and Pedersen commitments enables: Selective attribute disclosure, complex predicate proofs over committed values, Support for AND/OR/NOT logic, Range proofs and set membership.
        
\end{enumerate}

For Example 
$\phi$ the verification statement required by an employer during recruitment process for a new job requires a user to prove the following:
1. a valid master credential
2. a valid drivers license
3. an Australian University Degree
and show the university degree name

The user has previously obtained credentials, uses our modified PS signature scheme to rerandomize their credentials, and employs sigma protocols to prove valid master credential with respect to master credential issuer, etc
