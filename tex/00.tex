...
\begin{definition}[NP Relation and Language]
A relation $\mathcal{R} \subseteq \{0,1\}^* \times \{0,1\}^*$ is an NP relation if:
\begin{itemize}
    \item There exists a polynomial $p(\cdot)$ such that for all $(x,w) \in \mathcal{R}$, $|w| \leq p(|x|)$
    \item Membership in $\mathcal{R}$ can be verified in polynomial time
\end{itemize}
We define the language $\mathcal{L}_\mathcal{R} = \{x \mid \exists w : (x,w) \in \mathcal{R}\}$. Here, $x$ is called the statement and $w$ the witness.
\end{definition}

\sam{come back and build out the definition more}

\begin{definition}[Sigma Protocol]
A Sigma protocol for relation $\mathcal{R}$ is a three-move protocol between a prover $\mathcal{P}$ and verifier $\mathcal{V}$ with the following form:
\begin{itemize}
    \item Common input: statement $x$
    \item Prover's private input: witness $w$ such that $(x,w) \in \mathcal{R}$
\end{itemize}
The protocol proceeds as:
\begin{enumerate}
    \item $\mathcal{P}$ sends a commitment $a$ to $\mathcal{V}$
    \item $\mathcal{V}$ sends a random challenge $e \sample \{0,1\}^t$
    \item $\mathcal{P}$ sends a response $z$
\end{enumerate}
A transcript $(a,e,z)$ is accepting if $\mathcal{V}$'s verification predicate $\phi(x,a,e,z)=1$.
The protocol must satisfy:
\begin{itemize}
    \item Completeness: If $(x,w) \in \mathcal{R}$, then an honest $\mathcal{P}(x,w)$ convinces $\mathcal{V}(x)$ with probability 1
    \item Special Soundness: There exists a polynomial-time extractor $\mathcal{E}$ that, given any $x$ and accepting transcripts $(a,e,z)$ and $(a,e',z')$ with $e \neq e'$, outputs $w$ such that $(x,w) \in \mathcal{R}$
    \item Special Honest-Verifier Zero-Knowledge: There exists a PPT simulator $\mathcal{S}$ that, on input $x$ and challenge $e$, outputs $(a,e,z)$ with the same distribution as honest transcripts
\end{itemize}
\end{definition}
...
% Assume that the PS-LRSW assumption holds in the bilinear group $\mathbb{G}$. Then, in the Algebraic Group Model, our rerandomizable signature scheme over commitments is existentially unforgeable under adaptive chosen-commitment attacks. More precisely, for any algebraic PPT adversary $\mathcal{A}$, there exists a PPT reduction $\mathcal{B}$ such that
% \[
%     \Adv^{\mathsf{euf\mbox{-}cca}}_{\mathsf{RS},\mathcal{A}}(\lambda) \leq \Adv^{\mathsf{PS\mbox{-}LRSW}}_{\mathcal{B}}(\lambda) + \frac{q_v}{p},
% \]
% where $q_v$ is the number of verification queries.
% \end{theorem}

% \begin{proof}
% We describe a reduction $\mathcal{B}$ that uses an adversary $\mathcal{A}$ which breaks the EUF-CCA security of our scheme to solve an instance of the PS-LRSW problem. The reduction proceeds in three steps.

% \paragraph{1. Setup.}
% Given a PS-LRSW challenge instance
% \[
% (g, \tilde{g}, X = g^x, Y = g^y, \tilde{X} = \tilde{g}^x, \tilde{Y} = \tilde{g}^y),
% \]
% with unknown exponents $x,y \in \mathbb{Z}_p$, the reduction $\mathcal{B}$ does the following:
% \begin{enumerate}
%     \item Sets up the public parameters and commitment key $\mathsf{ck}$ as in the scheme.
%     \item Defines the public key $\mathsf{pk} = (\mathsf{ck},\vk)$ by setting $\vk$ (or a component of it) using the challenge elements (for example, letting $\vk = \tilde{X}$ or otherwise ensuring consistency with the verification equation).
%     \item Forwards $\mathsf{pk}$ (and any necessary group generators) to the adversary $\mathcal{A}$.
% \end{enumerate}
% The distribution of the public key is identical to that in the real scheme under the AGM.

% \paragraph{2. Simulation of Signing and Verification Oracles.}
% \begin{itemize}
%     \item \textbf{Signing Oracle:} For any signing query on a commitment $\cm$, $\mathcal{B}$ adds $\cm$ to the set $Q_{\cm}$ and simulates the signature using the PS-LRSW oracle. In particular, by querying the PS-LRSW oracle on $\cm$, $\mathcal{B}$ obtains a pair $(h, h^{x+m y})$, where $m$ is the message corresponding to $\cm$. This simulation is perfect because the response maintains the algebraic structure required by the scheme.
%     \item \textbf{Verification Oracle:} When $\mathcal{A}$ issues a verification query $(\sigma, \cm)$, assume that $\mathcal{A}$ provides algebraic representations of the signature:
%     \[
%     \sigma_1 = g^{\alpha_0}\prod_{i=1}^{n} g_i^{\alpha_i}h^{\alpha_{n+1}}, \quad \sigma_2 = g^{\beta_0}\prod_{i=1}^{n} g_i^{\beta_i}h^{\beta_{n+1}}.
%     \]
%     Then $\mathcal{B}$ checks whether the verification equation holds:
%     \[
%     e(\sigma_2, \tilde{g}) \stackrel{?}{=} e(\sigma_1, \tilde{X} \cdot \tilde{Y}^{m}),
%     \]
%     where $m$ is the message associated with the commitment $\cm$. The algebraic representations make it possible to verify this equation by comparing exponents.
% \end{itemize}

% \paragraph{3. Forgery and Extraction.}
% Eventually, $\mathcal{A}$ outputs a forgery $(m^*, \cm^*, \sigma^*)$, where $\cm^* \notin Q_{\cm}$. Write the forged signature as
% \[
% \sigma_1^* = g^{\gamma_0}\prod_{i=1}^{n} g_i^{\gamma_i}h^{\gamma_{n+1}}, \quad
% \sigma_2^* = g^{\delta_0}\prod_{i=1}^{n} g_i^{\delta_i}h^{\delta_{n+1}}.
% \]
% Because the forgery verifies, we have
% \[
% e(\sigma_2^*, \tilde{g}) = e(\sigma_1^*, \tilde{X}\cdot \tilde{Y}^{m^*}).
% \]
% By bilinearity, this implies that
% \[
% \delta_0 + \sum_{i=1}^{n}\delta_i + \delta_{n+1} = \Bigl(\gamma_0 + \sum_{i=1}^{n}\gamma_i + \gamma_{n+1}\Bigr)(x + m^*y).
% \]
% Since at least one of the coefficients $\gamma_i$ must be nonzero (else $\sigma_1^*$ would be trivial), $\mathcal{B}$ can select an index $i$ with $\gamma_i \neq 0$ and compute
% \[
% x + m^*y = \frac{\delta_i}{\gamma_i}.
% \]
% Given that $x$ and $y$ come from the PS-LRSW challenge, $\mathcal{B}$ can then extract
% \[
% m^* = \frac{1}{y}\Bigl(\frac{\delta_i}{\gamma_i} - x\Bigr).
% \]
% Finally, $\mathcal{B}$ outputs $(m^*, h^*, (h^*)^{x+m^*y})$, where $h^* = \sigma_1^*$, as a valid solution to the PS-LRSW problem.

% \paragraph{Analysis.}
% The simulation is perfect from the view of $\mathcal{A}$, and the probability that $\mathcal{A}$ outputs a valid forgery on a commitment not queried to the signing oracle is at least its advantage $\epsilon$ in the EUF-CCA game. The only additional error term arises from the possibility that $\mathcal{A}$ might output a trivial algebraic representation (which happens with probability at most $\frac{q_v}{p}$, where $q_v$ is the number of verification queries). Hence, the reduction satisfies
% \[
% \Adv^{\mathsf{euf\mbox{-}cca}}_{\mathsf{RS},\mathcal{A}}(\lambda) \leq \Adv^{\mathsf{PS\mbox{-}LRSW}}_{\mathcal{B}}(\lambda) + \frac{q_v}{p}.
% \]

% \paragraph{Discussion.}
% The AGM is essential here because it forces the adversary to produce signatures with explicit algebraic representations. This structure allows $\mathcal{B}$ to extract the exponents and, in turn, to solve for $m^*$. In the standard model, where adversaries might output arbitrary group elements without a decomposable structure, the extraction method used above would not apply directly.
...
% \newpage
% \begin{theorem}[EUF-CCA Security]
% In the Algebraic Group Model, if the PS-LRSW assumption holds in the bilinear group $\mathbb{G}$, then our signature scheme is existentially unforgeable under chosen message attacks. Specifically, for any algebraic PPT adversary $\mathcal{A}$, there exists a PPT reduction $\mathcal{B}$ against PS-LRSW such that:
% \[
%     \Adv^{\mathsf{euf\mbox{-}cca}}_{\mathsf{PS},\mathcal{A}}(\lambda) \leq \Adv^{\mathsf{PS\mbox{-}LRSW}}_{\mathcal{B}}(\lambda) + \frac{q_v}{p}
% \]
% where $q_v$ is the number of verification queries.
% \end{theorem}

% \begin{proof}
% We construct a reduction $\mathcal{B}$ that uses any algebraic adversary $\mathcal{A}$ breaking EUF-CCA to solve the PS-LRSW problem. The key insight is that the algebraic property lets us extract the structure of $\mathcal{A}$'s forgery directly.

% \paragraph{Setup:} On input PS-LRSW instance $(g, \tilde{g}, X = g^x, Y = g^y, \tilde{X} = \tilde{g}^x, \tilde{Y} = \tilde{g}^y)$, $\mathcal{B}$ proceeds as follows:
% \begin{enumerate}
%     \item Initialize signing query set $Q = \emptyset$
%     \item Forward generators and public key to $\mathcal{A}$: $(g, \tilde{g}, X, Y, \tilde{X}, \tilde{Y})$
% \end{enumerate}

% \paragraph{Signing Oracle:} On signing query for commitment $\cm$:
% \begin{enumerate}
%     \item Add $\cm$ to $Q$
%     \item Query PS-LRSW oracle on $\cm$ to obtain $(h, h^{x+my})$
%     \item Return signature to $\mathcal{A}$
% \end{enumerate}

% \paragraph{Verification Oracle:} On query $(\sigma, \cm)$, since $\mathcal{A}$ is algebraic, it provides representations:
% \[
% \sigma_1 = g^{\alpha_0}\prod_{i=1}^n g_i^{\alpha_i}h^{\alpha_{n+1}}, \quad
% \sigma_2 = g^{\beta_0}\prod_{i=1}^n g_i^{\beta_i}h^{\beta_{n+1}}
% \]
% Return 1 if verification equation holds using these representations.

% \paragraph{Forgery:} When $\mathcal{A}$ outputs $(\sigma^*, \cm^*)$, it must provide algebraic representations:
% \[
% \sigma_1^* = g^{\gamma_0}\prod_{i=1}^n g_i^{\gamma_i}h^{\gamma_{n+1}}, \quad
% \sigma_2^* = g^{\delta_0}\prod_{i=1}^n g_i^{\delta_i}h^{\delta_{n+1}}
% \]

% If $\cm^* \in Q$ or verification fails, abort. Otherwise, $\mathcal{B}$ extracts the PS-LRSW solution:
% \begin{enumerate}
%     \item Set $h^* = \sigma_1^*$ (this is the first component of PS-LRSW forgery)
%     \item Extract $m^*$ from the verification equation using the algebraic representations
%     \item Return $(m^*, h^*, (h^*)^{x+m^*y})$ as PS-LRSW forgery
% \end{enumerate}

% \paragraph{Analysis:} Let's analyze $\mathcal{B}$'s success probability in solving PS-LRSW using $\mathcal{A}$'s forgery.

% \paragraph{Perfect Simulation:} First, we show $\mathcal{B}$'s simulation is perfect from $\mathcal{A}$'s view:
% \begin{itemize}
%     \item The public key distribution is identical to the real scheme as it comes directly from the PS-LRSW instance
%     \item Signing queries are answered using the PS-LRSW oracle, producing valid signatures with the correct distribution
%     \item The verification oracle perfectly implements the verification equation using $\mathcal{A}$'s algebraic representations
% \end{itemize}

% \paragraph{Extraction Success:} When $\mathcal{A}$ produces a valid forgery $(\sigma^*, \cm^*)$, the verification equation must hold:
% \[
% e(\sigma_2^*, \tilde{g}) = e(\sigma_1^*, \tilde{X} \cdot \tilde{Y}^{m^*})
% \]

% By the algebraic property, this means:
% \[
% \delta_0 + \sum_{i=1}^n \delta_i + \delta_{n+1} = (\gamma_0 + \sum_{i=1}^n \gamma_i + \gamma_{n+1})(x + m^*y)
% \]

% Therefore, we have:
% \begin{itemize}
%     \item $h^* = \sigma_1^*$ is a valid group element (by $\mathcal{A}$'s success)
%     \item $m^*$ is extractable from the algebraic representations
%     \item $(h^*)^{x+m^*y} = \sigma_2^*$ by the verification equation
%     \item $\cm^* \notin Q$ (by definition of forgery)
% \end{itemize}

% \paragraph{Probability Analysis:} Let $\epsilon$ be $\mathcal{A}$'s advantage in the EUF-CCA game. Then:
% \[
% \Pr[\mathcal{B} \text{ succeeds}] \geq \epsilon - \Pr[\text{BadVer}]
% \]
% where $\text{BadVer}$ is the event that $\mathcal{A}$ produces a verification query that validates but doesn't correspond to its algebraic representation. By the q-type assumption underlying PS-LRSW, this probability is bounded by $q_v/p$.

% Therefore:
% \[
% \Adv^{\mathsf{PS\mbox{-}LRSW}}_{\mathcal{B}}(\lambda) \geq \Adv^{\mathsf{euf\mbox{-}cca}}_{\mathsf{PS},\mathcal{A}}(\lambda) - \frac{q_v}{p}
% \]
...
% \paragraph{Extracting $m^*$ from Algebraic Representations:} 
% Given $\mathcal{A}$'s forgery $(\sigma^*, \cm^*)$ with algebraic representations:
% \[
% \sigma_1^* = g^{\gamma_0}\prod_{i=1}^n g_i^{\gamma_i}h^{\gamma_{n+1}}, \quad
% \sigma_2^* = g^{\delta_0}\prod_{i=1}^n g_i^{\delta_i}h^{\delta_{n+1}}
% \]

% The verification equation provides:
% \[
% e(\sigma_2^*, \tilde{g}) = e(\sigma_1^*, \tilde{X} \cdot \tilde{Y}^{m^*})
% \]

% Substituting the algebraic representations and using bilinearity:
% \begin{align*}
% e(g^{\delta_0}\prod_{i=1}^n g_i^{\delta_i}h^{\delta_{n+1}}, \tilde{g}) &= e(g^{\gamma_0}\prod_{i=1}^n g_i^{\gamma_i}h^{\gamma_{n+1}}, \tilde{g}^x \cdot \tilde{g}^{ym^*}) \\
% &= e(g^{\gamma_0}\prod_{i=1}^n g_i^{\gamma_i}h^{\gamma_{n+1}}, \tilde{g}^{x+ym^*})
% \end{align*}

% This yields a system of equations in the exponents. For each generator basis:
% \begin{align*}
% \text{For } g&: \delta_0 = \gamma_0(x + ym^*) \\
% \text{For } g_i&: \delta_i = \gamma_i(x + ym^*) \text{ for } i \in [1,n] \\
% \text{For } h&: \delta_{n+1} = \gamma_{n+1}(x + ym^*) 
% \end{align*}

% Since $\mathcal{A}$ is algebraic, we can extract $m^*$ in one of two ways:

% \begin{enumerate}
%     \item \textbf{Direct Extraction:} From any non-zero coefficient $\gamma_i$:
%     \[
%     x + ym^* = \frac{\delta_i}{\gamma_i}
%     \]
%     Therefore:
%     \[
%     m^* = \frac{1}{y}(\frac{\delta_i}{\gamma_i} - x)
%     \]

%     \item \textbf{System Solution:} Using any two equations with non-zero coefficients:
%     \[
%     \frac{\delta_i}{\gamma_i} = \frac{\delta_j}{\gamma_j} = x + ym^*
%     \]
%     This provides a consistency check for the extracted value.
% \end{enumerate}

% Note that at least one $\gamma_i$ must be non-zero for a valid forgery, as otherwise $\sigma_1^*$ would be the identity element, violating the verification equation.
...
\begin{definition}[MIMC-ABC System] A Multi-Issuer Multi-Credential Attribute based Anonymous Credential system consists of the following $\PPT$ algorithms:
    ...
    \item $\mathsf{UserKeygen}(\secparam) \to (\usk)$ is a probabilistic algorithm. Outputs user secret key $\usk$ consisting of PRF key $k$ and identity string $s$
...
\begin{itemize}
    ...
    \item $\mathcal{O}_{\mathsf{Issue}}$ Protocol interaction where $\AdvA$ plays malicious user to obtain credentials from honest issuer
...
\begin{enumerate}
    ...
    \item $\AdvA$ can create/corrupt users, obtain credentials legitimately, and observe honest showings
    \item $\AdvA$ wins if it produces a valid showing for attributes not contained in any credential owned by corrupt users
\end{enumerate}
...
Anonymity models security against malicious issuers and verifiers. The definition gives the adversary $\AdvA$ (representing malicious issuer and verifier) access to oracles
\begin{itemize}
    \item $\mathcal{O}_{\mathsf{HU}}$ to create honest users, $\mathcal{O}_{\mathsf{CU}}$ to corrupt users
    \item $\mathcal{O}_{\mathsf{Obtain}}$: Protocol interaction where $\AdvA$ plays malicious issuer to issue credentials to honest users
    \item $\mathcal{O}_{\mathsf{Show}}$: Protocol interaction where $\AdvA$ plays malicious verifier observing credential showings
    \item $\mathcal{O}_{\mathsf{LoR}}$: The challenge oracle providing $\AdvA$ showing transcripts from either credential $j_0$ or $j_1$ based on challenge bit $b$
\end{itemize}
Note that the anonymity game lacks access to $\mathcal{O}{\mathsf{Issue}}$ because, as a malicious issuer, it can use its $(opk, osk)$ in a real protocol run, and lacks access to $\mathcal{O}_{\mathsf{ObtIss}}$ as that oracle models honest issuance. Instead, the anonymity game needs $\mathcal{O}_{\mathsf{Obtain}}$ to allow honest users to obtain credentials from the malicious issuer ($\AdvA$).

\noindent The Anonymity Game proceeds as follows
\begin{enumerate}
    \item $\AdvA$ generates (potentially) malicious organization keys $(opk, st) \gets \AdvA(\secparam)$
    \item $\AdvA$ can create honest users, issue them credentials (as a malicious issuer), and observe their showings (as a malicious verifier)
    \item For the challenge, $\AdvA$ selects 2 users' credentials and must distinguish which is being shown
    \item $\AdvA$ wins if it correctly guesses bit $b$ used by $\mathcal{O}_{\mathsf{LoR}}$
\end{enumerate}
...
\begin{enumerate}
    ...
    \item $\AdvA$ can create/corrupt users and obtain credentials legitimately from honest issuers
    \item $\AdvA$ wins if it produces a valid proof linking credentials that were issued to different user identifiers
\end{enumerate}
...
\newpage
\subsection{Proof of Knowledge for Pedersen Commitment Opening}
...
\subsection{AND proofs for protocol composition}
...
\newpage
\subsection{Proof of Knowledge for Linear Relations}
...
\newpage
\subsection{Proof of Knowledge of Zero Values}
...
\subsubsection{Zero-Value Proof Application to Selective Disclosure}
...
\subsection{Two-Party Secret Sharing Protocol}
\label{subsec:two-party-secret}
...
\subsection{Zero-Knowledge Proof of Non-Zero Secret}
...
\subsection{Equality Proofs Across Multiple Commitments}
...
\subsection{Signature of Knowledge with Rerandomizable PS Signatures}
...
Anonymous credential systems require mechanisms to prevent Sybil attacks without compromising user privacy. Traditional sybil-resistant schemes often reveal user information or relationships between credentials. Verifiable Random Functions (VRF) are often used as a solution as they generate a verifiably correct deterministic nullifier from user information for presentation to the issuer; prior schemes either reveal user information or relationships, or require heavy bilinear pairings introducing additional overhead. 

\noindent We improve the state of the art by creating a lightweight VRF proof protocol from committed attributes for applications in privacy preserving cryptography. We show it enables Sybil Resistance via
\begin{enumerate}
    \item \textbf{Adapted VRF Structure: } we redefine the Dodis-Yampolskiy VRF in standard prime-order groups, eliminating reliance on bilinear pairings while maintaining provable pseudorandomness under the $q$-DHI assumption
    
    \item \textbf{Composable Zero-Knowledge Proof}: we present a new $\Sigma$-protocol proving multiplicative inverse relations between committed values $m_1 = k + \textsf{ctx}$ and $m_2 = 1/m_1$, enabling verification of nullifier correctness without disclosure

    \item \textbf{Formalized Security: } reductions showing Sybil Resistance reduces to uniqueness from the VRF and extractability from the proof system
\end{enumerate}
...
A Verifiable Random Function (VRF) in prime-order group $\G$ of order $q$ is a tuple of PPT algorithms $(\mathsf{VRF.Gen}, \mathsf{VRF.Eval}, \mathsf{VRF.Vfy})$ with associated message space $\setX$, output space $\setY$, and proof space $\Pi$, defined as:
...
\begin{itemize}
    ...
    \item $\mathsf{VRF.Eval}(sk, x) \to y:$ Returns output $y \gets g^{1/(x+sk)}$
    
    \item $\mathsf{VRF.Vfy}(pk, x, y, \pi) \to \bit:$ validates proof $\pi$ that $y = g^{\frac{1}{x+sk}}$, outputs 1 for success, 0 for failure
\end{itemize}
\end{definition}
...
\subsection{Construction}
Our VRF construction builds upon the following primitives:

\paragraph{Building Blocks:}
\begin{itemize}
    \item Prime-order group $\G$ with generator $g$
    \item Pedersen commitments $\cm_k = g^k h^{r_1}$, $\cm_{\text{ctx}} = g^{\text{ctx}} h^{r_2}$
\end{itemize}

\paragraph{Algorithms:}
The VRF consists of two main algorithms:

\begin{enumerate}
    \item $\mathsf{Eval}(sk, \text{ctx}):$
    \begin{itemize}
        \item Compute $m = k + \text{ctx} \bmod q$
        \item Output nullifier $N = g^{1/m}$
    \end{itemize}

    \item $\mathsf{Verify}(pk, \text{ctx}, N, \pi):$
    \begin{itemize}
        \item Use $\Sigma$-protocol (Fig. 1) to prove:
        \[
            \exists (k, \text{ctx}, r_1, r_2) : \cm_k = g^k h^{r_1} \land
            \cm_{\text{ctx}} = g^{\text{ctx}} h^{r_2} \land N = g^{1/(k + \text{ctx})}
        \]
    \end{itemize}
\end{enumerate}

The security of this construction follows from the $q$-DHI assumption presented earlier and the binding property of Pedersen commitments. The $\Sigma$-protocol used in verification ensures zero-knowledge while maintaining the uniqueness property required for Sybil resistance.

\paragraph{Correctness:} The correctness of our VRF follows from the algebraic structure of the prime-order group and the properties of the Pedersen commitment scheme. For any valid key pair $(sk, pk)$ and context $\text{ctx}$, the verification equation will always be satisfied when evaluated honestly.

\paragraph{Security Analysis:} The security of our construction rests on three key properties:
\begin{enumerate}
    \item \textbf{Uniqueness:} Follows from the discrete logarithm assumption in $\G$
    \item \textbf{Pseudorandomness:} Reduces to the $q$-DHI assumption
    \item \textbf{Zero-Knowledge:} Inherited from the underlying $\Sigma$-protocol
\end{enumerate}
...
\newpage
\begin{protocol}{VRF Output Verification}{vrf-verify}
...
The special soundness property ensures that any prover who can produce valid proofs must know valid openings of the commitments satisfying the multiplicative inverse relation. This directly implies the uniqueness property of the VRF - two different VRF outputs would require different inverse relationships, contradicting the binding of the commitments.

The zero-knowledge property ensures that the proof reveals nothing about the committed values beyond their multiplicative inverse relationship, which is essential for the pseudorandomness of the VRF output. This separation between verifiability and secrecy allows us to achieve both properties simultaneously.
...
\newpage
We share a VRF with the issuer to ensure Sybil Resistance for the context as a nullifier.


\section{Proof of VRF Output from Committed Attributes}
Within our anonymous credential system, a user has a Master Credential with a PRF key $k$ committed as 
\[
\cm_{\textsf{m}} = \mathsf{CM.Com}([\textsf{id}, k, \textsf{ctx}]; r) = g_1^{\textsf{id}}g_2^kh^r
\]
A context credential is formatted as 
\[
\cm_{\textsf{c}} = \mathsf{CM.Com}([\textsf{id}, \textsf{ctx}]; r_2) = g_1^{\textsf{id}}g_2^{\textsf{ctx}}h^{r_2}
\]
...
We leverage the structure of the Dodis Yampolisky Verifiable Random Function (VRF)
\[
\text{Nullifier } = g^{1/k + \textsf{ctx}}
\]
The nullifier's correctness must be verifiable without revealing the key $\mathsf{k}$, the context type $\textsf{ctx}$, or their relationship which will be used during context credential generation, and can be used for revocation where the Nullifier may be added to a revocation list. 

\subsection{Reducing VRF consistency to a Multiplicative Inverse Proof}
The core challenge is to prove that the nullifier is derived from $\textsf{ctx}$ and $k$ in the credentials. To achieve this, we reparameterize the problem
\begin{itemize}
    % \item Let $\cm_1 = \mathsf{CM.Com}([k];r_1)$, Let $\cm_2 = \mathsf{CM.Com}([\mathsf{ctx}];r_2)$
    \item Let $m_1 = \mathsf{ctx} + k$ (the sum of the PRF key and context)
    \item let $m_2$ = $\frac{1}{m_1}$ (its multiplicative inverse)
    \item Let $\cm_1 = \mathsf{CM.Com}([m_1];r_1)$, Let $\cm_2 = \mathsf{CM.Com}([m_2];r_2)$
\end{itemize}

The $\textsf{Nullifier}$ is now $g^{m_2}$

To prove consistency, we show
\begin{enumerate}
    \item $\cm_1 = g^{m_1}h^{r_1}$
    \item $\cm_2 = g^{m_2}h^{r_2}$
    \item Auxilliary commitments $\cm_3 = \cm_1^{m_2}h^{r_3}$ and $\cm_4 = h^{r_4}$ enforce $m_1 \cdot m_2 = 1$ via
    \[
    \frac{\cm_3}{\cm_4} = g^{m1 \cdot m_2} = g^1 = g
    \]
    Which directly ties the VRF output $g^{m_2}$ to the validity of the multiplicative inverse relation
\end{enumerate}


\begin{theorem}
    proving $m_1 \cdot m_2 = 1$ is equivalent to verifying the correctness of the VRF under the hardness of the discrete log assumption
\end{theorem}
