...
\bottomrule
\end{tabular}
\caption{Analysis of verification costs. $P$ denotes pairing operation, $E_{\G_1}$ and $E_{\G_T}$ denote exponentiations in $\G_1$ and $\G_T$ respectively. $r$ denotes number of attributes.}
\end{table}
...
\subsection{Multi-Credential Verification}
Our focus requires proving knowledge of $N$ credentials with a shared attribute (e.g., $\mathsf{userid}$) under \textit{distinct public keys}. 
The PS-UTT variant enables this through:

\begin{itemize}
\item \textbf{Key Linking}: For credential $i$ with public key $\mathsf{pk}_i$, embed a shared Pedersen commitment to $\mathsf{userid}$:
\begin{equation}
C_{\mathsf{userid}} = g^{\mathsf{userid}}h^{r_i}
\end{equation}
where $r_i$ uses verifiably random blinding factors.

\item \textbf{Batch Proofs}: Given $N$ signatures $\{\sigma_i\}$ on $\{C_{\mathsf{userid}}^{(i)}\}$, prove in zero-knowledge:
\begin{equation}
\bigwedge_{i=1}^N \mathsf{Verify}(\mathsf{pk}_i, \sigma_i, C_{\mathsf{userid}}^{(i)}) \land \exists r,\mathsf{userid}: \bigwedge_{i=1}^N C_{\mathsf{userid}}^{(i)} = g^{\mathsf{userid}}h^{r_i}
\end{equation}
\end{itemize}

\subsubsection{Performance Advantages}
The PS-UTT combination provides:

\begin{itemize}
\item \textbf{Compact Proofs}: Using Groth16 SNARKs, proof size grows logarithmically with $N$ 
\item \textbf{Efficient Verification}: Pairing product checks for batched credentials:
\begin{equation}
\prod_{i=1}^N e(\sigma_i, \mathsf{pk}_i) = e\left(g, \prod_{i=1}^N H(C_{\mathsf{userid}}^{(i)})\right)
\end{equation}

\item \textbf{Practical Privacy}: Signatures are rerandomized as $\sigma'_i = \sigma_i^{1/\tau_i}$ for fresh $\tau_i$, breaking linkage between presentations
\end{itemize}
...
\begin{protocol}{Zero-Knowledge Proof of Knowledge of BBS+ Signature}{bbsplus-pok}
    \vspace{1em}
    ...
    \begin{enumerate}
        ...
        \item $\Prover$ computes responses:
            \begin{itemize}
                \item $z_e \gets r_e + c \cdot e$
                \item $z_s \gets r_s + c \cdot s$
                \item For $i = 1$ to $n$: $z_{m_i} \gets r_{m_i} + c \cdot m_i$
            \end{itemize}
            Send $(z_e, z_s, \{z_{m_i}\}_{i=1}^n)$ to $\Verifier$
    
        \item $\Verifier$ checks:
            \[
                A' \neq 1_{\G_1} \quad \wedge \quad e(A',\hat{w}) = e(\Bar{A},g_2)
            \]
            \[
                T_1 \stackrel{?}{=} A^{z_e} \cdot h_0^{z_s} \cdot (\Bar{A})^{-c}
            \]
            \[
                T_2 \stackrel{?}{=} g^c\prod_{i=1}^n h_i^{z_{m_i}}
            \]
    \end{enumerate}
\end{protocol}
...
\newpage
\subsection{BBS+ Original Computational Complexity}
...
\begin{itemize}
    ...
    \item \textbf{Batch Pairing Computation:} The Verifier computes $L+4$ pairings during verification. Instead of computing all pairings independently, batch pairing techniques can be applied to reduce overhead by precomputing shared terms. 
