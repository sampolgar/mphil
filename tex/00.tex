...
The master credential issuance protocol enables a user to obtain their root credential, a rerandomizable signature over commitments from the Master Credential Issuer $\mathcal{I}_{\mathsf{m}}.$ The Master Credential is modelled as a signature over commitment $\cmm = \mathsf{CM.Com}([\id, \ctx]; r)$

\todonote{keys: \ref{pok-keys}}
...
\begin{figure}
    ...
    $\underline{\mathsf{Obtain}(\usk, \opk)}$ && $\underline{\mathsf{Issue(\cm, \vec{m}, \osk)}}$ \\[1em]
    ...
    If $\cmm \neq \CMCom([\id, \ctx];\usk) \; $ or  & $\xleftarrow{\cmm, \id, \ctx, \sigmam}$ & $\sigmam \sample \RSSign(\cmm, \osk)$ \\[1em]
...
\newpage
\subsection{Correctness of Keys for protection against Malicious Issuer}\label{pok-keys}
