...
\newpage
\subsection{Unforgeability Base Case Single Issuer}
\begin{enumerate}
    \item \textbf{Setup:}
    \begin{itemize}
...
\newpage
\subsection{Unforgeability Base Case Single Issuer V2}
Adding $\phi$ satisfaction to it, satisfying zero-knowledge proof verification. 
I found it confusing using a rerandomizable signature scheme in the proofs because we wouldn't be tracking that anyway and can't be used for detecting forgeries.
\begin{itemize}
    \item Invariant parts of the credential is only $m = id, ctx$ and index $i$ is the owner.
    \item Variant = signature, commitment, commitment randomness $\usk$ and also signature randomness $u$
\end{itemize}

For a single-issuer, single-credential system, unforgeability means any adversary cannot produce a valid showing, consisting of a rerandomized credential and ZKP for a predicate $\phi$ unless they have corrupted a user whose issued credential satisfies $\phi$. 

\begin{enumerate}
    \item Type-1 Forgery: producing a showing for $\phi$ when no issued credential satisfies $\phi$
    \item Type-2 Forgery: producing a showing for $\phi$ using a credential whose attributes don't satisfy $\phi$
\end{enumerate}

\subsubsection{Forgery Goal}: $\adv$ outputs $\MIMCShow(i, \phi) \to (\sigma'^*, \cm'^*, \pi^*, \phi^*)$ where $\pi^*$ is a $\zkpok$ that $\cm'^*$ commits to some $m^*$ satisfying predicate $\phi^*$ and $\MIMCVerify(\sigma'^*, \cm'^*, \pi^*, \phi*) = 1$
A forgery fails if the $\adv$ relies on credentials they don't control (e.g. from $\HU$) or creates a showing for a predicate $\phi^*$ that no issued credential supports.
...
\begin{pchstack}[boxed, center, space=1em]
    \procedure[]{$\mathsf{Experiment}$ $\mathrm{Exp}^{\mathsf{UNF}}_{\mathsf{SingleIssuer}, \adv}(\lambda)$}{%
        ...
        \\
        \pccomment{Winning Condition} \\
        \Verify(\cred'^*, \cm'^*, \pi^*, \phi^*) = 1 \; \wedge\\
        \t \nexists (m, i) \in \CRED \text{ such that } \\
        \t i \in \CU \; \wedge \; m \text{ satisfies } \phi^*\\
    }
    \begin{pcvstack}
        \procedure[]{$\OHU(i)$}{%
            \pcif i \notin \HU \\ 
        ...
        \procedure[]{$\OCU(i)$}{%
            ...
            \t \text{possibly } \pcreturn \text{ all } (m,i) \text{ for }i \in \CRED\\
        }
        \procedure[]{$\OOBTISS(i, m)$}{%
            \pcif i \notin \HU, \pcreturn \bot \\
            \t \cm \gets \CMCom([m]; \usk[i]) \\
            \t \cred \gets (\Obtain(\usk[i], \cm), \Issue(\osk, \cm)) \\
            \t \CRED \gets \CRED \cup (m,i) \\
            % \t \CRED \gets \CRED \cup \{(\cred, \cm, m, i)\} \\
            % \t \OWNR[\cred] \gets i \\
            \pcreturn \cred \\
        }
...
\subsection{Unforgeability Single Issuer Reduction V2}
A successful forgery is defined as an Adversary $\adv$ who can produce a valid rerandomized credential and proof 
Unforgeability in the single-issuer context is the infeasibiltiy of $\adv$ producing a valid credential show from
1. a credential they don't control e.g. from another Honest User, or
2. a new proof $\pi^*$ that satisfies a predicate $\phi^*$ 

Forgery Goal: 
\begin{enumerate}
    \item Verification Success $(\MIMCVerify(\cred^*, \cm^*, \pi^*, \phi^*)=1$
    \\item $\nexists(m,i) \in \CRED$ such that $i \in \CU$ and $m$ satisfies $\phi$
    \item Credential mustn't be from a corrupt user (legitimate use of a corrupt user credential). $\nexists$ any credential $(m,i) \in \CRED$ such that $i \in \CU$ and $m$ satisfies $\phi^*$
    \item New credential, not previously issued. There shouldn't exist $(m,i) \in \CRED$ where $i \in \HU$ and $m$ satisfies $\phi^*$
\end{enumerate}

A forgery fails if
\begin{enumerate}
    \item Verify outputs 0 $(\MIMCVerify(\cred^*, \cm^*, \pi^*, \phi^*)=0$
    \item The forgery is from a corrupt user $i \in \CU$ but with a new $m^*$ that satisfies $\phi^*$
\end{enumerate}

Sets and terms
\begin{itemize}
    \item $\HU$ set of honest users, $\adv$ can't access.
    \item $\CU$ corrupt users, $\adv$ has secret keys and credentials
    \item $\CRED$ is a set of tuples $(m,i)$ where $m$ is the attributes and identifiers in the commitment and $i$ is the user index. Tracks all credentials
    \item Predicate $\phi^*$ is a condition or policy that $m$ satisfies for the showing to be valid
    \item $\MIMCShow$ takes in a tuple $(\cred'^*, \cm'^*, \pi^*, \phi^*)$
\end{itemize}
...
\begin{theorem}[Single Issuer Unforgeability V2]
The single-issuer anonymous credential system is unforgeable against $\mathrm{Exp}^{\mathsf{UNF}}_{\mathsf{SingleIssuer}, \adv}(\lambda)$ if the signature scheme is $\EUFCMA$, the commitment scheme is computationally binding, and the zero-knowledge proof system is computationally sound. 
\end{theorem}


\begin{proof}
    Assume there exists an adversary $\adv$ that can break the unforgeability of the single-issuer anonymous credential system with non-negligible probability $\epsilon$. That is, $\adv$ can produce a forged credential $\cred'^* = (\sigma'^*, \cm'^*, \pi^*, \phi^*)$ such that:
    \begin{itemize}
        \item $\MIMCVerify(\sigma'^*, \cm'^*, \pi^*, \phi^*) = 1$ and
        \item $\nexists (m,i) \in \CRED)$ such that $i \in \CU$ and $m$ satisfies $\phi^*$ (no signature message and user index $i$ in the list of issued creds $\CRED$ such that the user index $i$ is found in the corrupt users and the message $m$ is used to prove a new predicate $\phi^*$.
    \end{itemize}
    Then we construct simulators $\AdvB_1, \AdvB_2, \AdvB_3$ to reduce $\adv's$ advantage to breaking one of the underlying assumptions    
\end{proof}
...
\begin{theorem}[Single Issuer Unforgeability V2]
The single-issuer anonymous credential system is unforgeable as per $\mathrm{Exp}^{\mathsf{UNF}}_{\mathsf{SingleIssuer}, \adv}(\lambda)$ if the signature scheme is $\EUFCMA$, the commitment scheme is computationally binding, and the zero-knowledge proof system is computationally sound. 
\end{theorem}
...
\begin{proof}
    ...
    Then we construct an adversary $\advb$ the wins $\mathrm{Exp}^{\mathsf{UNF}}_{\mathsf{SingleIssuer}, \adv}(\lambda)$ with non-negligible probability. 
\end{proof}

\subsubsection{Setup}
$\AdvC$ sets up the $\EUFCMA$ challenge $\sk, \vk$, generates $\OSIGN(\sk)$, shares $\vk$ and access to $\OSIGN(\cdot)$ with $\advb$. $\advb$ receives $\vk$, oracle access $\OSIGN(\cdot)$ and sets $\opk = \vk$

\subsubsection{Oracle Simulation}
$\advb$ simulates the following oracles
\begin{itemize}
    \item $\OHU(i)$: generate $\usk_i$, adds $i$ to $\HU$
    \item $\OCU(i)$: generate $\usk_i$, give $\usk_i$ to $\adv$, add $i$ to $\CU$
    \item $\OOBTAIN(i,m)$: 
    \begin{itemize}
        \item If $i \in \HU: $ Retrieve $\usk_i$ from $\HU$. Compute $\cm = \CMCom(m; \usk_i)$. Query $\sigma = \OSIGN(\cm)$. Simulate issuing with $\sigma$
        \item If $i \in \CU: $ Receive $\cm_1, \pirzero$ from $\adv$. $\ZKVerify(\pirzero, \cm_1) = 1$ such that $\cm_1 = \CMCom(0; \usk_i)$. Compute $\cm = \cm_1 \cdot \CMCom([m];0)$. Query $\sigma \gets \OSIGN(\cm)$. Return $\sigma$.
    \end{itemize}
    \item $\OSHOW(i, \phi): $ if $i \in \HU$, use $\usk_i$ to rerandomize and generate $\ZKP$
\end{itemize}

\subsubsection{Forgery:}
\begin{itemize}
    \item $\adv$ outputs $(\sigma'^*, \cm'^*, \pi^*, \phi^*)$ where $\MIMCVerify(\sigma'^*, \cm'^*, \pi^*, \phi^*) = 1$ and $\nexists (m,i) \in \CRED $ with $i \in \CU$ where $m$ satisfies $\phi^*$
    \item By $\ZKP$ soundness, $\pi^*$ implies there exists $m^*, r^*$ such that $\cm = \CMCom(m^*;r^*)$ and $\RSVer(\sigma'^*, \cm'^*, \vk) = 1$ and $m^*$ satisfies $\phi^*$
    \item $\cm^*$ doesn't correspond to any $\cm$ issued to corrupt users
    \item $\adv$ doesn't know $\usk_i$ for honest users by the commitment hiding property, $\adv$ can't guess $r^*$ for $\HU$ credentials
    \item Thus $\cm^*$ was not queried to $\OSIGN$ making $\cm'^*, \sigma'^*$ a valid break against $\EUFCMA$
\end{itemize}
