...
How do we use credentials today? Gartner says it will open new use cases
- new loan, need multiple credentials
...
% Line spacing for the document

\begin{protocol}{MPCitH-VE\textsubscript{R}}
Converts the MPCitH-IOP prover $\mathcal{P}$ and verifier $\mathcal{V}$ to an MPCitH-VE prover $\mathcal{P}$ and verifier $\mathcal{V}$ using the extractable commitment scheme $\text{ECOM} = (\text{CGen}, \text{Commit}, \text{CExt})$.

\textbf{Parameters:} The number of parties $N$; the number of parallel repetitions $\tau$; ...

\textbf{Key Generation:} $\mathcal{G}(1^\kappa)$ invokes $(\text{pk}, \text{sk}) \leftarrow \text{CGen}(1^\kappa)$ and ...

\textbf{Two-party protocol} $\langle \mathcal{P}(w), \mathcal{V} \rangle (\text{pk}, x)$:
\begin{enumerate}[label=\arabic*.]
    \item $\mathcal{P}$ runs $\mathcal{P}$ on input $(x, w)$ ...
    \item $\mathcal{P}$ separately commits ...
\end{enumerate}
...
\begin{protocol}
\textbf{Protocol 2: MPCitH-VE\textsubscript{R}}
...
\subsection{Protocol Description (This section was done previously, needs updating!}
...
\begin{theorem}
    Construction \ref{construct:openproof} is a $\Sigma-$construction for the relation: 
    \[
