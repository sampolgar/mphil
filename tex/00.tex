...
\subsection{Security Model before}
...
%         \ck \gets \CMSetup(\secparam) \\
%         (\osk, \opk) \gets \OrgKeyGen(\ppar) \\
%         \HU \gets \emptyset, \; \CU \gets \emptyset, \; \CRED_M \gets \emptyset, \; \CRED_C \gets \emptyset \\
%         \pccomment{CRED_M, CRED_C store tuples (id_i, attrs, i, cm, σ)} \\
%         \\
%         \pccomment{Adversary Interaction} \\
%         \AdvA^{\OHU, \OCU, \OOBTMASTER, \OOBTCONTEXT, \OSHOW}(\opk, \ck) \\
%         \AdvA \text{ outputs } (\credm'^*, \credc'^*, \cmm'^*, \cmc'^*, \pi^*, \phi^*) \\
%         \\
%         \pccomment{Winning Condition} \\
%         \MIMCVerify(\credm'^*, \credc'^*, \cmm'^*, \cmc'^*, \pi^*, \phi^*) = 1 \; \wedge \\
%         \t \nexists\, i \in \CU \text{ such that there exist:} \\
%         \t\t (\id_i, \attrsm, i, \cmm, \sigmam) \in \CRED_M \; \wedge \\
%         \t\t (\id_i, \attrsc, i, \cmc, \sigmac) \in \CRED_C \; \wedge \\
%         \t\t \phi^*((\id_i, \attrsm), (\id_i, \attrsc)) = 1 \\
%     }
%     \begin{pcvstack}
%         \procedure[]{$\OHU(i)$}{%
...
%             \t \HU \gets \HU \cup \{i\} \\
%             \t \usk[i] \gets \UserKeyGen(\ppar) \\
%             \t \id_i \gets \text{random unique identifier} \; \pccomment{e.g., sample or hash i} \\
%         }
%         \procedure[]{$\OCU(i)$}{%
%             \pcif i \notin \HU, \t \CU \gets \CU \cup \{i\} \\
%             \pcelse, \t \HU \gets \HU \setminus \{i\}, \; \CU \gets \CU \cup \{i\} \\
%             \t \pcreturn (\usk[i], \{(id_i, attrs, i, cm, σ) \in \CRED_M \cup \CRED_C\}) \\
%         }
%         \procedure[]{$\OOBTMASTER(i, \attrsm)$}{%
%             \pcif i \notin \HU, \pcreturn \bot \\
%             \t \cmm \gets \CMCom([\id_i, \attrsm]; \usk[i]) \\
...
%         \procedure[]{$\OOBTCONTEXT(i, \attrsc, \credm)$}{%
%             \pcif i \notin \HU, \pcreturn \bot \\
%             \pcif \credm \notin \{(\sigmam, \cmm) \mid (\id_i, \attrsm, i, \cmm, \sigmam) \in \CRED_M\}, \pcreturn \bot \\
...
%             \t (\id_i, \attrsc, i, \cmc, \sigmac) \in \CRED_C \text{ s.t. } \phi((\id_i, \attrsm), (\id_i, \attrsc)) = 1 \\
%             \t \pcif \text{no pair exists}, \pcreturn \bot \\
%             \t (\credm', \credc', \cmm', \cmc', \pi) \gets \Show(\sigmam, \cmm, \sigmac, \cmc, \usk[i], \phi) \\
%             \pcreturn (\credm', \credc', \cmm', \cmc', \pi) \\
%         }
%     \end{pcvstack}
% \end{pchstack}
