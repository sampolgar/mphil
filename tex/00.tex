...
We address the problem space created by the fundamental tension between properties in a private identity system: privacy, accountability, and functionality. 
To resolve these tensions, we build privacy-preserving protocols that achieve accountability through Sybil resistance and revocation while maintaining functionality via complex identity support - all validated through concrete implementations, benchmarks, and formal security analysis in the game-based model.
...
\subsection{Remaining Issues in current system / the gap}
% Sybil Resistance with multiple credentials, Credential Chaining, Revocation.


Evolution of Identity SYstems and the new paradigm
- Traditional Systems: single identity provider, limited usage scope to single organisation, or multiple approved organisations with SSO ecosystem
- Transitioning to a decentralized model: a user with multiple credentials from diverse issuers
- Emergence of credential wallets holding multiple credentials from different sources
- Novel functionality: combining multiple credentials for complex verifications

Institutional Integration and Challenges
- Traditional Institutions joining the DID ecosystem
- Need for minimal infrastructure changes, cryptographic implementations, and supporting infrastructure
- Balance existing systems with DID capability
- Requiring interoperability between legacy and new systems
- Requiring to adhere to regulations

The new Frontier: Credential Oracles and Attestation Services
- The new paradigm expands beyond traditional credential issuers
- There is a rise in attestation services like TLS Notary, DECO, ZKPassport, etc
- There is an emerging theme where attested data can be a credential which presents challenges in
- - sybil resistance of these attestations and credential types
- - maintaining accountability in a decentralized system
- - preserving privacy while enabling verifications and adhering to regulations
- - revocation capabilities without building new infrastructure
- - new use cases are showing up, unlocking a users value. Identity using multiple credentials at the same time, Private AI showing up related to identity systems



Core Challenges

Integration of Privacy Preserving Primitives
- Combining revocation, sybil resistance and privacy preserving protocols cohesively together while retaining security properties of an identity system
- Designing efficient zero knowledge proof protocols that enable multiple primitives to work together efficiently
- Efficient protocol composition

Privacy Vs Accountability in Public Verifier Environment
- enabling mass-scale public verifiability without centralised trust while distributing trust
- Maintaining unlinkability across multiple credential presentations with accountability
- Supporting public verifiability and revocation without compromising privacy

Sybil Resistance
- sybil resistance with credential oracles 

Secure Credential Composition
- credential chaining for complex verification scenarios while adhereing to regulations

Revocation
- Efficient non-membership proof for any number of credentials (batch proof)


Limitations / Gap
- Functionality of Private Identity Systems requires combining cryptographic primitives together. Many primitives are implemented in isolation and lose security properties when composed together in a protocol. 


Sybil Resistance
- Efficient, Privacy Preserving Sybil Resistance is challenging
- Combined with low overhead on computational setup i.e. many issuers don't need to run revocation list themselves
- Sybil Resistance is not discussed in the context of Attested Data/Oracle based Credentials 


Revocation
- Efficient non-membership proof for any number of credentials (batch proof)

Gaps in Secure Credential Composition
- Current SOTA may provide the above properties but do not support efficient composition and support complex verification scenarios
...
We identify three fundamental technical challenges in building a privacy-preserving DID system with Anonymous Credentials:
...
\begin{enumerate}
    ...
    Efficient Private Revocation: Interaction with existing revocation approaches either compromise anonymity or require expensive cryptographic accumulators that limit scalability.
...
Features    									& Sybil Resistance		& Efficient Cred. Chaining$\footnotemark[1]$ & M-ABC$\footnotemark[2]$         & Anonymity$\footnotemark[3]$     & Revocation\\
\hline
...
S3ID \cite{rabaninejad_attribute-based_nodate}  & \ding{51}    	& \ding{51}     & \ding{51}\footnotemark[4]     & \ding{51}		& \ding{55}\\
zk-creds \cite{rosenberg_zk-creds_2022}  & \ding{51}    	& \ding{51}     & \ding{51}     & \ding{51}		& \ding{55}\\
...
\footnotesize
$\footnotemark[4]$ Efficient Credential Chaining 
\end{table}
