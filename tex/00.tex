...
\subsection{Extended Properties (Multi-Credential)}
\begin{itemize}
...
\newpage
\section{Unforgeability: Single Issuer, Single Credential}
...
\paragraph{Example 1 (Credential Forgery):} An adversary produces a credential $\cred = (\sigma, \cm = \CMCom([\text{age}=17];\usk))$  that passes signature verification $\RSVer(\sigma,\cm,\vk)=1$ without being issued by the authority. This directly breaks the underlying signature scheme.

\paragraph{Example 2 (Predicate Misuse):} A corrupt user with a valid credential for age = 17
...
\begin{pchstack}[boxed, center, space=1em]
    \procedure[]{$\mathsf{Experiment}$ $\mathrm{Exp}^{\mathsf{UNF}}_{\mathsf{SingleIssuer}, \adv}(\lambda)$}{%
        ...
        \t \nexists (m, i, \cm, \sigma) \in \CRED \text{ such that } \\
        \t i \in \CU \; \wedge \; \phi^*(m) = 1\\
    }
    \begin{pcvstack}
        \procedure[]{$\OHU(i)$}{%
            \pcif i \notin \HU \\ 
        ...
        \procedure[]{$\OCU(i)$}{%
            \pcif i \in \HU \land i \notin \CU \\
            \t \CU \gets \CU \cup \{i\} \\
            \t \pcreturn \usk[i] \; \wedge \\
            \t \text{optionally } \pcreturn \text{ all } (m,i) \text{ for }i \in \CRED\\
        }
        ...
        \t \cred \gets (\sigma, \cm) \\
        \t \CRED \gets \CRED \cup (m, i, \cm, \sigma) \\
        \pcreturn \cred \\
    }
        \procedure[]{$\OSHOW(i, \phi)$}{%
        \pcif i \notin \HU, \pcreturn \bot \\
        \t \text{Find all } (m, i, \cm, \sigma) \in \CRED \text{ where } \phi(m) = 1 \\
        \t \pcif \text{no such credential exists}, \pcreturn \bot \\
        \t \pcelse \text{Select one such credential } (m, i, \cm, \sigma) \\
        \t \t (\cred', \cm', \pi) \gets \Show(\sigma, \cm, \usk[i], \phi) \\
        \pcreturn (\cred', \cm', \pi) \\
    }
    \end{pcvstack}
\end{pchstack}
...
\begin{itemize}
    ...
    \item $\OOBTISS(i, m)$: For user $i$ and message $m$, $\advb$ computes $\cm = \CMCom(m; \usk_i)$, obtains $\sigma \gets \OSIGN(\cm)$, records $(m, i, \cm, \sigma)$ in $\CRED$, and returns $(\sigma, \cm)$.
    
    \item $\OSHOW(i, \phi)$: For honest user $i$ and predicate $\phi$, $\advb$ finds credentials in $\CRED$ whose attributes satisfy $\phi$, generates a valid showing, and returns it.
\end{itemize}


\noindent \textbf{Forgery:} When $\adv$ outputs forgery $(\cred'^*, \cm'^*, \pi^*, \phi^*)$ satisfying:
\begin{align*}
\MIMCVerify(\cred'^*, \cm'^*, \pi^*, \phi^*) = 1 \quad \wedge \quad \nexists (m, i, \cm, \sigma) \in \CRED : i \in \CU \wedge \phi^*(m) = 1
\end{align*}
...
\subsection{Progression to Multi-Credential Unforgeability}
...
\begin{enumerate}
    \item \textbf{Credential Attribute Manipulation}: Modifying attributes within otherwise legitimate credentials
...
We formalize multi-credential unforgeability through an experiment where an adversary interacts with credential oracles and outputs a credential showing. The winning condition requires:

\begin{align}
    &\MIMCVerify(\credm'^*, \credc'^*, \cmm'^*, \cmc'^*, \pi^*, \phi^*) = 1 \quad \wedge\\
    & \nexists\, i \in \CU \text{ such that } \\
    &(id_i, \attrsm, i, \cdot, \cdot) \in \CRED_M \quad \wedge \\
    &(id_i, \attrsc, i, \cdot, \cdot) \in \CRED_C \quad \wedge \\
...
\clearpage
\subsection{Unforgeability: Single-Issuer, Multi-Credential}
...
\begin{pchstack}[boxed, center, space=1em]
    \procedure[]{$\mathsf{Experiment}$ $\mathrm{Exp}^{\mathsf{UNF}}_{\mathsf{SingleIssuer\text{-}MultiCred}, \adv}(\lambda)$}{%
        ...
        \pccomment{CREDi: (id, attrs, user, commitment, signature)} \\
        \\
        \pccomment{Adversary Interaction} \\
    ...
    \procedure[]{$\OOBTCONTEXT(i, \attrsc, \credm)$}{%
        ...
        \pcif \nexists(\id_i, \attrsm, i, \cmm, \sigmam) \in \CRED_M \\
        \t \textsf{with } \credm = (\sigmam, \cmm), \pcreturn \bot \\
        \t \pccomment{Create $\credc$ bound to $\id$} \\
        \t \cmc \gets \CMCom([\id_i, \attrsc]; \usk[i]) \\
        \t \pccomment{Simulate $\pireq(\cmm, \cmc)$} \\
        \t \pireq \gets \ZKProve(\cmm, \cmc, \id_i, \phi_\mathsf{eq})\\
        \t \sigmac \gets \Issue(\osk, \cmc, \cmm, \pi_{eq}) \\
        \t \credc \gets (\sigmac, \cmc) \\
        \t \CRED_C \gets \CRED_C \cup (id_i, \attrsc, i, \cmc, \sigmac)  \\
        \pcreturn \credc \\
    }
    ...
    \procedure[]{$\OSHOW(i, \phi)$}{%
        \pcif i \notin \HU, \pcreturn \bot \\
        \t \text{Find } (id_i, \attrsm, i, \cmm, \sigmam) \in \CRED_M \text{ and } \\
        \t (id_i, \attrsc, i, \cmc, \sigmac) \in \CRED_C \text{ such that } \\
        \t \phi((id_i, \attrsm), (id_i, \attrsc)) = 1 \\
        \t \pcif \text{no such credential pair exists}, \pcreturn \bot \\
        \t \t (\credm', \credc', \cmm', \cmc', \pi) \gets \\
        \Show(\sigmam, \cmm, \sigmac, \cmc, \usk[i], \phi) \\
...
\newpage
\subsection{Completing the Single-Issuer, Multi-Credential Unforgeability Reduction}

\subsubsection{1. Formalized Oracle Specifications}

First, let's precisely define your oracles with explicit tracking of credential relationships:

\begin{pcvstack}[boxed, center, space=1em]
    \procedure[]{$\mathsf{Setup}$ and $\mathsf{KeyGeneration}$}{%
        \ppar \gets \Setup(\secparam) \\
        \ck \gets \CMSetup(\secparam) \\
        (\osk, \opk) \gets \OrgKeyGen(\ppar) \\
        \HU \gets \emptyset, \; \CU \gets \emptyset \\
        \CRED_M, \CRED_C \gets \emptyset, \emptyset \\
        \OWNR \gets \emptyset \pccomment{Maps credential indices to user identities} \\
    }
    
    \procedure[]{$\OHU(i)$}{%
        \pcif i \in \HU \cup \CU, \pcreturn \bot \\
        \usk[i] \sample \UserKeyGen(\secparam) \\
        \HU \gets \HU \cup \{i\} \\
        \pcreturn \opk \\
    }
    ...
    \procedure[]{$\OCU(i)$}{%
        ...
        \CU \gets \CU \cup \{i\} \\
        \pcreturn \usk[i] \text{ and all } \text{CRED}_j \text{ where } \OWNR[j] = i \\
    }
    
    \procedure[]{$\OOBTMASTER(i, \attrsm)$}{%
        \pcif i \notin \HU, \pcreturn \bot \\
        \id_i \sample \{0,1\}^\lambda \pccomment{Generate unique identifier} \\
        \cmm \gets \CMCom([\id_i, \attrsm]; \usk[i]) \\
        \sigmam \gets \Issue(\osk, \cmm) \\
        \credm \gets (\sigmam, \cmm) \\
        j \gets |\CRED_M| + 1 \pccomment{New credential index} \\
        \CRED_M \gets \CRED_M \cup \{j\} \\
        \OWNR[j] \gets i \\
        \ATTR_M[j] \gets (\id_i, \attrsm) \\
        \pcreturn \credm \\
    }
    
    \procedure[]{$\OOBTCONTEXT(i, \attrsc, j_m)$}{%
        \pcif i \notin \HU, \pcreturn \bot \\
        \pcif j_m \notin \CRED_M \lor \OWNR[j_m] \neq i, \pcreturn \bot \pccomment{Verify master credential ownership} \\
        (\id_i, \attrsm) \gets \ATTR_M[j_m] \pccomment{Extract identity from master credential} \\
        \cmc \gets \CMCom([\id_i, \attrsc]; \usk[i]) \\
        \pccomment{Formalized binding proof verification} \\
        \pi_{eq} \gets \ProveEq(\cmm_{j_m}, \cmc, \id_i, \usk[i]) \\
        \pcif \VerifyEq(\pi_{eq}, \cmm_{j_m}, \cmc) = 0, \pcreturn \bot \\
        \sigmac \gets \Issue(\osk, \cmc) \\
        \credc \gets (\sigmac, \cmc) \\
        j_c \gets |\CRED_C| + 1 \\
        \CRED_C \gets \CRED_C \cup \{j_c\} \\
        \OWNR[j_c] \gets i \\
        \ATTR_C[j_c] \gets (\id_i, \attrsc) \\
        \LINK[j_c] \gets j_m \pccomment{Track which master credential authorized this context credential} \\
        \pcreturn \credc \\
    }
\end{pcvstack}

\subsubsection{2. Verification Algorithm Formalization}

\begin{pchstack}[boxed, center, space=1em]
    \procedure[]{$\MIMCVerify(\credm', \credc', \cmm', \cmc', \pi, \phi)$}{%
        \pccomment{Signature verification on both credentials} \\
        \pcif \RSVer(\credm', \cmm', \opk) = 0, \pcreturn 0 \\
        \pcif \RSVer(\credc', \cmc', \opk) = 0, \pcreturn 0 \\
        \pccomment{Verify ZK proof demonstrating:} \\
        \pccomment{1. Both commitments are well-formed} \\
        \pccomment{2. Both commitments share the same identity} \\
        \pccomment{3. Committed attributes satisfy predicate} \\
        \pcif \ZKVer(\pi, \cmm', \cmc', \phi) = 0, \pcreturn 0 \\
        \pcreturn 1 \\
    }
    
    \procedure[]{$\ZKVer(\pi, \cmm', \cmc', \phi)$}{%
        \pccomment{Verify that $\pi$ proves the following statement:} \\
        \exists \id, \attrsm, \attrsc, r_m, r_c \text{ such that:} \\
        \cmm' = \CMCom([\id, \attrsm]; r_m) \land \\
        \cmc' = \CMCom([\id, \attrsc]; r_c) \land \\
        \phi((\id, \attrsm), (\id, \attrsc)) = 1 \\
    }
\end{pchstack}

\subsubsection{3. Complete Reduction Framework}

\begin{proof}
We construct a reduction from the unforgeability of our multi-credential system to the $\EUFCMA$ security of the underlying signature scheme. Given an adversary $\adv$ breaking our definition with probability $\epsilon$, we build an adversary $\advb$ breaking $\EUFCMA$.

\textbf{Setup:} $\advb$ receives $\vk$ and oracle access to $\OSIGN(\cdot)$ from the $\EUFCMA$ challenger. It sets $\opk = \vk$ and simulates our experiment.

\textbf{Oracle Simulation:}
$\advb$ implements $\OHU$, $\OCU$ as described. For $\OOBTMASTER$ and $\OOBTCONTEXT$, $\advb$ uses $\OSIGN(\cdot)$ to obtain signatures, carefully tracking all credential data in $\CREDM$, $\CREDC$, $\OWNR$, $\ATTRM$, $\ATTRC$, and $\LINK$.

\textbf{Forgery Extraction:} When $\adv$ outputs $(\credm'^*, \credc'^*, \cmm'^*, \cmc'^*, \pi^*, \phi^*)$ satisfying our winning condition, $\advb$ proceeds with case analysis:

\textbf{Case 1:} If either $\cmm'^*$ or $\cmc'^*$ is not a rerandomization of any commitment in our records, $\advb$ returns the corresponding signature as an $\EUFCMA$ forgery.

\textbf{Case 2:} If both credentials are rerandomizations of existing commitments but from different users:
\begin{itemize}
    \item By the binding property of commitments and soundness of the ZK system, the proof $\pi^*$ must demonstrate that both commitments share the same identity $\id^*$.
    \item However, the original commitments contained different identities, creating a contradiction.
    \item Therefore, the adversary must have broken either commitment binding or ZK soundness to produce $\pi^*$.
\end{itemize}

\textbf{Case 3:} If both credentials are rerandomizations from the same corrupt user but don't satisfy $\phi^*$:
\begin{itemize}
    \item The ZK proof incorrectly claims that attributes satisfy $\phi^*$, contradicting soundness.
\end{itemize}

In all cases, we either extract a direct $\EUFCMA$ forgery or establish a contradiction with a fundamental security property, completing our reduction.
\end{proof}



\subsection{Progression to Multi-Issuer, Multi-Credential Unforgeability}
