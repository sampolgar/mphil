...
A signature scheme $(\mathsf{KeyGen}, \mathsf{Sign}, \mathsf{Verify})$ is existentially unforgeable under adaptive chosen-message attacks if for all PPT algorithms $\mathcal{A}$ with access to a signing oracle $\mathsf{Sign}(\cdot, \mathsf{sk})$ there is a negligible function $\negl$ such that:

$$\Pr\left[\begin{array}{l}
    (\mathsf{sk}, \mathsf{pk}) \stackrel{\$}{\leftarrow} \mathsf{KeyGen}(1^k), \\
    (m^*, \sigma^*) \stackrel{\$}{\leftarrow} \mathcal{A}^{\mathsf{Sign}(\cdot,\mathsf{sk})}(\mathsf{pk})
\end{array} : m^* \notin Q \wedge \mathsf{Verify}(m^*, \sigma^*, \mathsf{pk}) = 1\right] \leq \negl$$
...
\[
    ...
    \right | \leq \negl
\]
Informally, hiding states that a commitment $\cm$ hides the underlying messages, even a computationally bounded adversary $\Adv$ can not distinguish between commitments with different messages of their choice with probability better than random guessing, even when $\Adv$ chooses both messages.
\end{definition}
...
\begin{definition}[Binding] A rerandomizable vector commitment scheme $\mathsf{RVC}$ is binding if for all $\PPT$ adversaries $\Adv$ and all message vectors $\vec{m}_0, \vec{m}_1$ of equal length, there is a negligible function $\negl$ such that:
    \[
        ...
        \right ] \leq \negl
    \]
Informally, No adversary $\Adv$ should be able to open a commitment to two different messages, even when allowed to use different randomness values. This captures the "unforgeability" aspect of commitments. 
\end{definition}
...
\[
    ...
    \right] \leq \negl
\]
Informally: No adversary $\Adv$ should be able to open a vector commitment to two different messages, differing at a specific position $i$. This ensures that when using commitment openings in proof systems, the following two commitments are not equal $\mathsf{CM.Com}([\alpha, \beta]; r_1)$ $\neq$ $\mathsf{CM.Com}([\beta, \alpha]; r_1)$.
\end{definition}
...
\begin{theorem}[Position Binding from SDLP]
If the Symmetric Discrete Logarithm (SDLP, \ref{sdlp}) assumption holds in the bilinear group, then our RVC scheme satisfies position binding.
\end{theorem}

\begin{proof}
We proceed by contradiction through a reduction to SDLP. Suppose there exists a PPT adversary $\mathcal{A}$ that breaks position binding with non-negligible probability $\epsilon$. We construct a PPT adversary $\mathcal{B}$ that breaks the SDLP assumption with probability $\epsilon/\ell$ (where $\ell$ is the vector length). 
% why do we use epsilon over ell? 
% epsilon is the probability that B breaks position binding
% so we split it between the number of messages because it could break any of the messages?

$\mathcal{B}$ receives an SDLP challenge $(g^x, \tilde{g}^x)$ and needs to compute $x$. It simulates the position binding game for $\mathcal{A}$ as follows:

% Simulating the position binding game for A means it runs the position binding definition 

1. $\mathcal{B}$ picks a random index $i^* \sample [1,\ell]$ where it expects $\mathcal{A}$ to break position binding

2. For the commitment key generation: 
   - For position $i^*$: sets $g_{i^*} \gets g^x, \tilde{g}_{i^*} \gets \tilde{g}^x$ from the challenge
   - For all other positions $j \neq i^*$: samples $y_j \sample \Z_p$ and sets $g_j \gets g^{y_j}, \tilde{g}_j \gets \tilde{g}^{y_j}$

3. Provides $\ck = (g, (g_1,\ldots,g_\ell), \tilde{g}, (\tilde{g}_1,\ldots,\tilde{g}_\ell))$ to $\mathcal{A}$

4. $\mathcal{A}$ outputs $(\cm, i, m, m', r, r')$ where:
   - $\cm$ is a commitment
   - $i$ is the broken position
   - $m, m'$ are different messages for position $i$
   - $r, r'$ are the respective openings

5. If $i \neq i^*$, $\mathcal{B}$ aborts. Otherwise:
   - Given valid openings, we have:
     $\cm = g^r\prod_{j=1}^\ell g_j^{m_j} = g^{r'}\prod_{j=1}^\ell g_j^{m'_j}$
   - At position $i^*$, this gives us:
     $g^r(g^x)^m\prod_{j\neq i^*} g_j^{m_j} = g^{r'}(g^x)^{m'}\prod_{j\neq i^*} g_j^{m'_j}$
   - Since $m_j = m'_j$ for $j \neq i^*$, we can solve for $x$:
     $x = \frac{r' - r}{m - m'} \bmod p$

$\mathcal{B}$ succeeds whenever $\mathcal{A}$ breaks position $i^*$, which occurs with probability $\epsilon/\ell$ when $\mathcal{A}$ succeeds (probability $\epsilon$) and $\mathcal{B}$ guesses the correct position (probability $1/\ell$).

The key insight is that the symmetric property $e(\cm, \tilde{g}) = e(g, \widetilde{\cm})$ ensures that breaking position binding in either group leads to breaking SDLP, as any valid opening must be consistent across both groups.

Therefore, if $\mathcal{A}$ succeeds with non-negligible probability $\epsilon$, then $\mathcal{B}$ breaks SDLP with non-negligible probability $\epsilon/\ell$, contradicting the SDLP assumption.
\end{proof}
