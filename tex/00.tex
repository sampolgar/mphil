...
\subsection{Construction}
We instantiate our VRF with a prime-order group $\G$ and generator $g$, we assume the existence of two pedersen commitments, with at a minimum, key $k$ in one, and $\textsf{ctx}$ in the other, for example, $\cm_k = g^k h^{r_1}$, $\cm_{\textsf{ctx}} = g^{\textsf{ctx}} h^{r_2}$. In practice, the commitments may have a different structure but should reduce to this format. 

\noindent Let $\G$ be a prime-order group with generator $g$, our VRF scheme consists of
\begin{itemize}
    \item $\mathsf{Eval}(k, \textsf{ctx}):$ Compute $m_1 = k + \text{ctx} \bmod q$, output Nullifier $\textsf{N} = g^{1/m}$

    \item $\mathsf{Prove}(pk, \textsf{ctx}, \textsf{N}, \Pi^{\mathcal{R}_{\textsf{VRF}}}):$ use $\Sigma$-protocol to prove the relation 
    \[
...
\paragraph{Core Reduction}  
The Sybil resistance mechanism reduces VRF verification to proving a multiplicative inverse relationship between committed values. Specifically, the $\Sigma$-protocol in Protocol~\ref{proto:vrf-verify} demonstrates that:
\begin{itemize}
    \item The nullifier $\textsf{N} = g^{m_2}$ is derived from $m_2 = 1/m_1$, where $m_1 = k + \textsf{ctx}$.
    \item The auxiliary commitments $\cm_3, \cm_4$ enforce $m_1 \cdot m_2 = 1$ algebraically, ensuring the nullifier’s structural correctness.
\end{itemize}
...
\paragraph{Connection to VRF Security (Revised)}  
