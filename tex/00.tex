...
\begin{itemize}
    ...
    % https://claude.ai/chat/d6f264cd-4cf4-45ad-ae98-be8204443936
    We extend the challenge oracle of \cite{fuchsbauer_structure-preserving_2019} for multi-credential verification. 
    
    

%     OLoR({j0[k]}k∈[n], {j1[k]}k∈[n], φ) is the challenge oracle in the anonymity game where the adversary must distinguish (multiple) showings of credential sets CRED[j0[k]] and CRED[j1[k]]. The oracle takes two sequences of issuance indices {j0[k]}k∈[n], {j1[k]}k∈[n] and a predicate φ. If JLoR ≠ ∅ and JLoR ≠ {{j0[k]}k, {j1[k]}k}, it returns ⊥. Let i0 ← OWNR[j0[1]] and i1 ← OWNR[j1[1]]. 
    
%     For k ∈ [2,n], if OWNR[j0[k]] ≠ i0 or OWNR[j1[k]] ≠ i1, return ⊥. If JLoR = ∅, then it sets JLoR ← {{j0[k]}k, {j1[k]}k} and ILoR ← {i0, i1}. If i0, i1 ∉ HU or ¬(φ({ATTR[j0[k]]}k) ∧ φ({ATTR[j1[k]]}k)), it returns ⊥. Else, it runs
% (S, ·) ←$ (Show(opk, φ, {CRED[jb[k]]}k), ·),
...
\textbf{Oracle} $\mathcal{O}_{\mathsf{LoR}}(\mathsf{Creds}_0, \mathsf{Creds}_1, \phi)$

\smallskip
\textit{Input:}
\begin{itemize}
    \item Two credential tuples $\mathsf{Creds}_b = (cred_{b,1},...,cred_{b,n})$ for $b \in \{0,1\}$
    \item A verification predicate $\phi: \mathsf{ATTR}^n \rightarrow \{0,1\}$ that validates multiple credential attributes
\end{itemize}

\textit{Pre-conditions:}
\begin{enumerate}
    \item No active challenge: $\mathcal{J}_{\mathsf{LoR}} = \emptyset$ or $\mathcal{J}_{\mathsf{LoR}} = \{\mathsf{Creds}_0, \mathsf{Creds}_1\}$
    
    \item Single ownership: For each credential tuple, extract owners
    \begin{itemize}
        \item Let $i_b \gets \mathsf{OWNR}(cred_{b,1})$ for $b \in \{0,1\}$
        \item Require $\forall k \in [2,n]: \mathsf{OWNR}(cred_{b,k}) = i_b$
    \end{itemize}
    
    \item Honest users: $i_0, i_1 \in \mathsf{HU}$
    
    \item Valid predicate satisfaction: 
    \begin{itemize}
        \item Let $\mathsf{attrs}_b = (\mathsf{ATTR}(cred_{b,1}),...,\mathsf{ATTR}(cred_{b,n}))$
        \item Require $\phi(\mathsf{attrs}_0) = 1 \wedge \phi(\mathsf{attrs}_1) = 1$
    \end{itemize}
\end{enumerate}

\textit{If any pre-condition fails, return $\bot$. Otherwise:}
\begin{enumerate}
    \item Set $\mathcal{J}_{\mathsf{LoR}} \gets \{\mathsf{Creds}_0, \mathsf{Creds}_1\}$
    \item Set $\mathcal{I}_{\mathsf{LoR}} \gets \{i_0, i_1\}$
    \item Execute $(S, \cdot) \sample (\mathsf{Show}(opk, \phi, \mathsf{Creds}_b), \cdot)$
\end{enumerate}
\end{minipage}
}
\caption{Challenge Oracle for Multi-Credential Anonymity}
\label{fig:oracle-lor}
\end{figure}
...
\end{definition}
\begin{definition}[Credential Binding]
