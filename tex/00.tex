...
\begin{definition}[Symmetric Discrete Logarithm Assumption (SDLP)]
...
\subsection{Signature Adaptation Under Malicious Keys}
...
\begin{itemize}
    ...
    \item $\mathsf{RS.Ver}(\vk, \cm, \sigma) \to \bit:$
        Parse $\sigma$ as $(\sigma_1, \sigma_2)$,
        Assert $e(\mathsf{cm}, \tilde{g}) = e(g, \widetilde{\mathsf{cm}})$ and 
...
\subsubsection{Formal Rerandomization Process}
Explicitly define signature rerandomization:
\[
\sigma' = \left(\sigma_1^{u_\Delta},\, \left(\sigma_2 \cdot \sigma_1^{r_\Delta}\right)^{u_\Delta}\right)
\]
Prove validity preservation:
\[
e(\sigma_2', \tilg) = e(\sigma_1', \tilx \cdot \widehat{\cm}') \implies e\left((\sigma_2 \cdot \sigma_1^{r_\Delta})^{u_\Delta}, \tilg\right) = e\left(\sigma_1^{u_\Delta}, \tilx \cdot (\widehat{\cm} + r_\Delta\widehat{H})\right)
\]
...
\sam{come back to prove correctness}
