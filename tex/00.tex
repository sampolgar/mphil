...
An ABC system is correct if for all $\lambda \in $ for all credential tuples (cred₁,...,credₙ) honestly generated via ObtIss, and for all valid predicates φ, the following holds:
\begin{enumerate}
    \item Individual Validity: Each credential verifies independently
    \item Composition Validity: Any valid combination of credentials from the same user verifies successfully
    \item Attribute Preservation: The showing protocol preserves committed attributes
\end{enumerate}
More formally, for any PPT adversary $\Adv$, we require:
\[ Pr[Exp^{corr}_{\mathcal{ABC},\mathcal{A}}(1^\lambda) = 1] = 1 \]
\end{definition}
