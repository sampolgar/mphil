...
\newpage
\section{Symmetric Bilinear Rerandomizable Vector Commitments}
...
% Basic definitions, notation, properties

\subsection{Interactive Proofs and Zero-Knowledge}

Interactive proofs allow a prover $\mathcal{P}$ to convince a verifier $\mathcal{V}$ of the validity of some statement, while potentially keeping certain information private. In the context of anonymous credentials, these proofs serve as fundamental building blocks for demonstrating possession of credentials without revealing unnecessary information.

\subsection{Motivating $\Sigma$-Protocols}

The canonical example motivating $\Sigma$-protocols is Schnorr's identification scheme. Consider a prover who knows the discrete logarithm $w$ of a public group element $h$ with respect to generator $g$ in a group $\mathbb{G}$ of prime order $q$. Formally, the relation is:

\[
    \mathcal{R}_{\mathsf{DL}} = \{(h,w) \in \mathbb{G} \times \mathbb{Z}_q : h = g^w\}
\]

\begin{protocol}{Schnorr's Protocol}{schnorr}
\textbf{Common Input:} Group element $h \in \mathbb{G}$\\
\textbf{Prover Input:} $w \in \mathbb{Z}_q$ such that $h = g^w$
\begin{enumerate}
    \item $\mathcal{P}$ samples $r \sample \Z_q$, computes $a = g^r$
    \item $\mathcal{V}$ samples challenge $e \sample \{0,1\}^t$ where $2^t < q$
    \item $\mathcal{P}$ computes $z = r + ew \mod q$
    \item $\mathcal{V}$ accepts if $g^z = a \cdot h^e$
\end{enumerate}
\end{protocol}

This protocol exhibits three important properties:

\begin{itemize}
    \item \textbf{Knowledge Soundness:} Given transcripts $(a,e,z)$ and $(a,e',z')$ where $e \neq e'$, one can efficiently extract $w = (z-z')(e-e')^{-1} \mod q$
    \item \textbf{Special Honest-Verifier Zero-Knowledge:} For any fixed challenge $e$, one can efficiently simulate accepting transcripts by sampling $z \sample \Z_q$ and computing $a = g^z h^{-e}$
    \item \textbf{Three-Move Structure:} The protocol follows a commitment-challenge-response pattern that enables efficient composition
\end{itemize}

These properties form the basis for defining $\Sigma$-protocols, a framework that captures Schnorr's protocol and many other practical proof systems. The framework provides precise definitions and composition theorems while retaining the efficiency of the underlying protocols.
...
\subsection{Relations and Protocol Structure}

In constructing zero-knowledge proofs, we begin by precisely defining what it means to prove knowledge of some secret information. Following Damgård's formalization, we work with binary relations $R \subseteq \{0,1\}^* \times \{0,1\}^*$. For $(x,w) \in R$, we call $x$ the statement and $w$ the witness. The relation must be polynomially bounded: if $(x,w) \in R$, then $|w| \leq p(|x|)$ for some polynomial $p(\cdot)$.

For example, in your credential system, a typical relation might be:
\[
    \mathcal{R}_{\mathsf{CM}} = \left\{ (\mathsf{cm}, (m,r)) \;\middle|\; \mathsf{cm} = g^{m}h^r \right\}
\]
where $\mathsf{cm}$ is a Pedersen commitment and $(m,r)$ constitute its opening.

\subsection{$\Sigma$-Protocol Template}
A $\Sigma$-protocol follows a specific three-move structure, traditionally called "commitment-challenge-response":

\begin{protocol}{$\Sigma$-Protocol Template}{sigma-template}
\textbf{Common Input:} Statement $x$\\
\textbf{Prover Input:} Witness $w$ such that $(x,w) \in R$
    \begin{enumerate}
        \item $\mathcal{P}$ computes first message $a$ (the commitment) and sends it to $\mathcal{V}$
        \item $\mathcal{V}$ sends a random $t$-bit challenge $e$
        \item $\mathcal{P}$ computes response $z$ and sends it to $\mathcal{V}$
...
A $\Sigma$-protocol for relation $R$ is a three-move public-coin protocol between a prover $\mathcal{P}$ and verifier $\mathcal{V}$ that follows our template structure while satisfying three specific properties. Let's examine each property carefully:
...
An important subtlety in this definition is the "special" nature of both the soundness and zero-knowledge properties. Special soundness requires extraction from any pair of accepting transcripts with the same first message, which is stronger than standard knowledge extraction. Similarly, special HVZK requires simulation for any given challenge, not just a random one.

For a relation $R$, we denote by $L_R$ the language
\[
    L_R = \{x \mid \exists w : (x,w) \in R\}
\]

A crucial consequence of special soundness is that any $\Sigma$-protocol for $R$ is also an interactive proof system for language $L_R$ with soundness error $2^{-t}$ where $t$ is the challenge length. This follows because any prover successfully responding to more than one challenge must know a witness.
...
\subsection{Core Proof Protocols}
% Individual atomic proofs used in your system

\subsection{Compound Statements}
% How the basic proofs are combined

\subsection{Protocol-Specific Proofs}
% Your master and context credential specific proof constructions
...
\subsection{Proof of Multiplicative Inverse}
\sam{Build this out because it's my contribution, Add complexity analysis comparing this to standard equality proofs or standard VRF, Include metrics, communication cost and operation count}

\sam{Show the difference between mine and the one in UTT, Formalise security proof as main theorem}
