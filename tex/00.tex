...
\begin{itemize}
    ...
    \]
        ...
        \[
        \mathcal{R}_{\mathsf{verkey}} = \{(\pk = (\vk, \ck), (x, \{y_i\}_{i=1}^\ell)): 
        \vk = \tilde{g}^x \wedge
...
\begin{definition}[MIMC-ABC System] A Multi-Issuer Multi-Credential Attribute-based Anonymous Credential system consists of the following $\PPT$ algorithms:
    ...
    \item $(\mathsf{Obtain}(\vec{m}, \usk, \opk,  \phi, aux), \mathsf{Issue}(\osk, \cm, \phi, \{\Pi\}_i, aux)) \rightarrow (\cred, \bot)$ is an interactive protocol between a user and an issuing organization. Can be used 
    for obtaining a master or context credential $\credm, \credc$. $\mathsf{Obtain}$ The user inputs their message vector $\vec{m}$ and $\usk$, generates a commitment $\cm = \CMCom([\vec{m}];\usk)$ and proof of knowledge satisfying statement $\phi$. Obtaining a Context Credential requires verifying a master credential and consistency of user id $\id$ between master and context which we represent as auxiliary information $aux$. $\mathsf{Issue}$ Issuer runs this with their secret key $\osk$, commitment $\cm$, verifies proofs $\{\Pi\}_i$ against the statement $\phi$. 
    The protocol outputs a credential $\cred$ to the user and $\bot$ to the issuer.
...
A Multi-Issuer Multi-Credential Anonymous Credential system provides credential binding if no $\PPT$ adversary $\mathcal{A}$ can successfully combine credentials from different user identities into a valid proof. Specifically, $\mathcal{A}$ cannot:
\begin{enumerate}
...
The master credential issuance protocol enables a user to obtain their root credential, a rerandomizable signature over commitments from the Master Credential Issuer $\mathcal{I}_{\mathsf{m}}.$ The Master Credential is modelled as a signature over commitment $\cm_m = \mathsf{CM.Com}([\id, \ctx]; r)$
...
\begin{figure}
    ...
    \multicolumn{3}{l}{$\underline{\mathsf{(Obtain, Issue)}}: \pirzero(\cmm = \CMCom([0,0];\usk)) = \zkpok\{(\cmm, \usk)| \cmm = g_1^0g_2^0g^{\usk}\}$}\\[1em]
    \multicolumn{3}{l}{$\Pi^{\mathcal{R}_{\mathsf{opk}}}(\sk, \vk) = \zkpok\{(x, \sk, r) | \cm = g^xh^r \wedge \tilde{cm} = \tilde{g}^x\tilde{h}^r e(\sk,\vk) = 1\G_T\} \mathsf{VerKeyGen}$} \\[1em]
    % user proves they created an empty commitment, organisation proves they created their key correctly
    $\underline{\mathsf{(Obtain)}}$ && $\underline{\mathsf{(Issue)}}$ \\[0.5em]
...
Given the commitment key and secret and verification key, we require the issuer to prove in zero knowledge the correctness of their key computation to prevent malicious key usage that could break cryptographic primitives like the commitment scheme and signature scheme.
\begin{itemize}
    \item $\mathsf{CM.Setup:} \ck \gets (g, (g_1,\ldots,g_\ell), \tilde{g}, (\tilde{g}_1,\ldots,\tilde{g}_\ell))$
...
We want to prove the following relation:
\[
\mathcal{R}_O = \{(\pk = (\vk, \ck), \sk): 
    \exists x, \{y_i\}_{i=1}^\ell \text{ s.t. }
    sk = g^x \wedge vk = \tilde{g}^x \wedge
    \bigwedge_{i=1}^\ell (g_i = g^{y_i} \wedge \tilde{g}_i = \tilde{g}^{y_i})\}
\]
...
\begin{enumerate}
    \item \textbf{Commitment:} Prover samples $\alpha, \{\beta_i\}_{i=1}^\ell \sample \Z_p$ and computes:
    \begin{itemize}
        \item $T_1 = g^\alpha, \tilde{T}_1 = \tilde{g}^\alpha$
        \item For $i \in [1,\ell]: T_i = g^{\beta_i}, \tilde{T}_i = \tilde{g}^{\beta_i}$
    \end{itemize}
    Send $\{T_1, \tilde{T}_1, \{T_i, \tilde{T}_i\}_{i=1}^\ell\}$ to verifier.
    ...
    \end{itemize}
    Send $(z_x, \{z_i\}_{i=1}^\ell)$ to verifier.
    
    \item \textbf{Verification:} Verifier checks:
    \begin{align*}
        g^{z_x} &\stackrel{?}{=} T_1 \cdot sk^c\\
