...
\begin{itemize}
    \item \textbf{Prover's Initial Operations (Signature Randomization)}
        \begin{itemize}
            \item $\sigma_1' = \sigma_1^r$ \qquad $1E_{\G_1}$
            \item $\sigma_2' = (\sigma_2 \cdot \sigma_1^t)^r$ \qquad $2E_{\G_1} + 1M_{\G_1}$
        \end{itemize}
        
    \item \textbf{Prover Schnorr Operations}
        \begin{itemize}
            \item Sample $\ell+2$ field elements ($t, r, m_1,\ldots,m_\ell$) \qquad (negligible)
            \item Compute commitment $T$
            \[
            T \gets e(\sigma_1',\hat{X}) \cdot \prod_{j=1}^\ell e(\sigma_1', \widehat{Y}_j)^{\tilde{m}_j} \cdot e(\sigma_1', \hat{g})^{\tilde{t}}
            \]
                \begin{itemize}
                    \item $e(\sigma_1',\hat{X})$ \qquad $1P$
                    \item $e(\sigma_1', \hat{g})^{\tilde{t}}$ \qquad $1P + 1E_{\G_T}$
                    \item $e(\sigma_1', \widehat{Y}_j)^{\tilde{m}_j}$ \qquad $\ell (P + E_{\G_T})$
                    \item Multiply all terms \qquad $(\ell+1 )M_{\G_T}$
                \end{itemize}
                \item Compute commitment $T$: \qquad $(\ell+2)P + (\ell+1)E_{\G_T} + (\ell+1) M_{\G_T}$
        \end{itemize}
        
    \item \textbf{Proof Size / Data Sent}
        \begin{itemize}
            \item $(\sigma_1', \sigma_2')$ \qquad $2\G_1$ elements
            \item $T$ \qquad $1\G_T$ element
            \item $z_{x}$ for $x \in \{t,r,m_1,\ldots,m_\ell\}$ \qquad $(\ell+2)\Z_p^*$ elements
        \end{itemize}
        
    \item \textbf{Verifier Operations}
        \begin{itemize}
            \item Final equation verification: \qquad $(\ell+3)P + (\ell+1)E_{\G_T} + (\ell +2) M_{\G_T}$
                \begin{itemize}
                    \item $e(\sigma_2', \hat{g})$ \qquad $1P$
                    \item $e(\sigma_1',\hat{X})$ \qquad $1P$
                    \item $e(\sigma_1', \widehat{Y}_j)^{z_{m_j}}$ \qquad $\ell P + \ell E_{\G_T}$
                    \item $e(\sigma_1', \hat{g})^{z_t}$ \qquad $1P + 1E_{\G_T}$
                    \item Final multiplications \qquad $(\ell +2 )M_{\G_T}$
                \end{itemize}
        \end{itemize}
\end{itemize}
...
\newpage
\subsection{PS \cite{tomescu2022utt}}
...
\begin{protocol}{Zero-Knowledge Proof of Knowledge for PS Signature over Commitments}{ps-pok}
    ...
    \begin{enumerate}
        ...
        \begin{itemize}
            ...
            \[
                T_1 \gets h^{\tilde{r'}} \prod_{i=1}^\ell g_i^{\tilde{m_i}}
            \]
            \[
                T_2 \gets e(\sigma_1^{\tilde{u'}}, \widehat{X} \cdot \widehat{\cm'})
            \]
...
\item \textbf{Verifier Operations}
    \begin{itemize}
        ...
        \item Second equation check: $e(\sigma_2, \hat{h})^c \cdot T_2 \stackrel{?}{=} e(\sigma_1^{z_{u'}}, \widehat{X} \cdot \widehat{\cm'})$ \qquad $ 1E_{\G_1} + 1M_{\G_2} + 1E_{\G_T} + 2P + 2M_{\G_T} $
    \end{itemize}
\end{itemize}
