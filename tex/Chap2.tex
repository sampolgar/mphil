...
The key $\k$ in the master credential, used as the input to a Verifiable Random Function (VRF), is generated through a two-party protocol between the user and the issuer. This process enhances unforgeability by preventing the user from forging a new master credential using a previously known key $\k$. 

The protocol proceeds as follows:
\begin{enumerate}
    \item The user samples a secret $\k_1 \sample \Z_p$ and commits to it via a commitment scheme.
    \item The issuer samples a secret $\k_2 \sample \Z_p$ and embeds it into the credential's attribute vector.
    \item  The final key is computed as $\k = \k_1 + \k_2 \mod p$, ensuring neither party alone knows $\k$.
\end{enumerate}

This collaborative approach ensures the key $\k$ remains unpredictable and uncontrollable by the user, bolstering security against forgery attacks.
