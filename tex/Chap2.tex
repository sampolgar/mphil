\mychapter{Efficient Multi-Show Attribute-Based (Anonymous) Credentials (ABC's)}\label{chap2}
...
\subsubsection{Dual-Group Construction}
...
\section{Rerandomizable Position Binding Pedersen Commitments}
...
% \begin{proof}[Proof Sketch]
% We embed an SDLP instance $(g^x, \tilde{g}^x)$ at a random position and use the adversary’s position binding break to extract the discrete logarithm $x$. Given an SDLP instance $(g^x, \tilde{g}^x) \in \G_1 \times \G_2$, $\mathcal{B}$ constructs a commitment key $\mathsf{ck}$:
% \begin{enumerate}
%     \item Choose $i^* \sample [1,n]$ uniformly.
%     \item Set $(g_{i^*}, \tilde{g}_{i^*}) = (g^x, \tilde{g}^x)$; for $j \neq i^*$, sample $y_j \sample \Z_p$ and set $(g_j, \tilde{g}_j) = (g^{y_j}, \tilde{g}^{y_j})$.
%     \item Output $\mathsf{ck} = (g, (g_1, \ldots, g_n), \tilde{g}, (\tilde{g}_1, \ldots, \tilde{g}_n))$.
% \end{enumerate}

% Run $\mathcal{A}(\mathsf{ck})$, which outputs $(\mathsf{cm}, \widetilde{\mathsf{cm}}, i, \attrs_0, \attrs_1, r_0, r_1)$ breaking position binding at position $i$, where $\attrs_0[i] \neq \attrs_1[i]$ and $\attrs_0[j] = \attrs_1[j]$ for $j \neq i$. 

% Since $\mathcal{A}$ is algebraic, it provides representations:
% \begin{enumerate}
%     \item $\mathsf{cm} = g^{\alpha} \prod_{k=1}^n g_k^{\beta_k}$
%     \item $\widetilde{\mathsf{cm}} = \tilde{g}^{\tilde{\alpha}} \prod_{k=1}^n \tilde{g}_k^{\tilde{\beta}_k}$
% \end{enumerate}

% From $\mathsf{Open}$, we have:
% \begin{enumerate}
%     \item $\mathsf{cm} = g^{r_0} \prod_{k=1}^n g_k^{m_{0,k}} = g^{r_1} \prod_{k=1}^n g_k^{m_{1,k}}$
%     \item $\widetilde{\mathsf{cm}} = \tilde{g}^{r_0} \prod_{k=1}^n \tilde{g}_k^{m_{0,k}} = \tilde{g}^{r_1} \prod_{k=1}^n \tilde{g}_k^{m_{1,k}}$
%     \item $e(\mathsf{cm}, \tilde{g}) = e(g, \widetilde{\mathsf{cm}})$
...
% Substituting $g_k = g^{y_k}$ (and $g_{i^*} = g^x$), equate exponents in $\G_1$:

% $$r_0 + \sum_{k \neq i} y_k m_{0,k} + x m_{0,i} = r_1 + \sum_{k \neq i} y_k m_{1,k} + x m_{1,i}.$$

% Since $m_{0,k} = m_{1,k}$ for $k \neq i$, this simplifies to:

% $$r_0 + x m_{0,i} = r_1 + x m_{1,i} \implies x (m_{0,i} - m_{1,i}) = r_1 - r_0.$$

% If $i = i^*$, solve:

% $$x = \frac{r_1 - r_0}{m_{0,i^*} - m_{1,i^*}} \mod p,$$

% where $m_{0,i^*} \neq m_{1,i^*}$ ensures division is defined. The pairing check confirms consistency across $G_1$ and $G_2$. $\mathcal{B}$ succeeds with probability $\epsilon / n$, where $\epsilon$ is $\mathcal{A}$'s advantage, completing the reduction.
% \end{proof}
...
\begin{itemize}
    ...
    \item \textbf{Position Binding:} Under the SDLP assumption in the AGM, the scheme is position binding. The reduction embeds an SDLP instance at a random position, using the adversary’s algebraic break to extract the solution with advantage scaled by $1/n$.
...
Our G2 signature variant \ref{rerandsig_g2} represents an optimization over the PS-UTT G1 approach described in \cite{tomescu_utt_2022}. By relocating signature components from $\G_1$ to $\G_2$, we achieve verification speedups and complete Show + Verify speedup by reducing a pairing operation, we prove correctness below \ref{rerandsig_g2}.  Table~\ref{tab:g1-g2-comparison} presents a direct comparison between the two variants.
