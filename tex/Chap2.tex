\mychapter{Improved Constructions of Anonymous Credentials from Rerandomizable Signatures on Commitments}\label{chap2}
ABC 
Anonymity Against Malicious Issuer
...
\section{Formal Treatment for UTT's Rerandomizable Signature Over Commitments}\label{sec:rerandsig_g1}
...
\section{Extending UTT's Construction}
...
\subsection{Verify Speedup Improvement}

In the original PS-UTT $\G_1$ variant, verification involves two bilinear pairings equality checks:
$$
e\bigl(\sigma_{2}',\tilde g\bigr)
= e\bigl(\sigma_{1}',\,\vk\cdot\widetilde{\cm'}\bigr),
\quad
e\bigl(\cm',\tilde g\bigr)
= e\bigl(g,\,\widetilde{\cm'}\bigr).
$$
By redefining the signature as
$$
\widetilde\sigma = \bigl(\widetilde\sigma_1,\widetilde\sigma_2\bigr)\in\G_2,
$$
the G2 variant replaces both checks with a single pairing:
$$
e\bigl(g,\,\widetilde\sigma_2'\bigr)
= e\bigl(\vk\cdot\cm',\,\widetilde\sigma_1'\bigr).
$$
This reduction eliminates one pairing operation, yielding a $20\%\text{--}40\%$ decrease in verification cost (see Table~\ref{abc-performance-combined-table}).
...
\begin{enumerate}
  \item \emph{Signature check:}
    $$
    e\bigl(g,\,\widetilde\sigma_2'\bigr)
    = e\bigl(\vk\cdot\cm',\,\widetilde\sigma_1'\bigr).
    $$
  \item \emph{Proof-of-knowledge check:}
    $$
    \mathsf{ZK.Verify}(\pi,\cm')=1,
    \quad
    \pi\;\biglarrow\;\mathsf{PoK}\bigl\{(r+\Delta_r,m_1,\dots,m_\ell):
    \cm'=g^{r+\Delta_r}\prod_{i=1}^{\ell}g_i^{m_i}\bigr\}.
    $$
\end{enumerate}
...
\begin{aligned}
e\bigl(g,\widetilde\sigma_2'\bigr)
&= e\bigl(g,(\widetilde\sigma_2\cdot\widetilde\sigma_1^{\Delta_r})^{\Delta_u}\bigr)\\
&= e\bigl(g,\widetilde\sk^{\Delta_u}\bigr)\;\,
   e\bigl(g,\widetilde\cm^{u+\Delta_u}\bigr)\;\,
   e\bigl(g,\tilde h^{r_\Delta\Delta_u}\bigr)\\
&= e\bigl(g^x,\tilde h^{\Delta_u}\bigr)\;\,
   e\bigl(g,\widetilde\cm\bigr)^{u+\Delta_u}\;\,
   e\bigl(g,\tilde h^{\Delta_u}\bigr)^{r_\Delta}\\
&= e\bigl(\vk\cdot\cm',\widetilde\sigma_1'\bigr).
\end{aligned}
$$
...
\section{Expressive Proof Evaluation}
...
Our G2 signature variant \ref{rerandsig_g2} represents an optimization over the PS-UTT G1 approach described in \cite{tomescu_utt_2022}. By relocating signature components from $\G_1$ to $\G_2$, we achieve verification speedups and complete Show + Verify speedup by reducing a pairing operation. 
