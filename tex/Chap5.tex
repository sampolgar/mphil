...
\begin{enumerate}
    \item $|S|$ issuers run 
    \begin{enumerate}
        \item $\ZKVerify(\cmm, \cmc, \nullifier, \Pi^{\mathsf{R}_{\mathsf{credc}}})$ and $\ZKVerify(\{\cm_k, \Pi_k^{\mathsf{zkpok}}\}_{k \in [\ell]})$\footnote{Both proofs use the same verifier challenge, they are separated here to aid understanding}
...
\subsection{Note on Deduplication Consistency}
\label{subsec:dedup-consistency}

For sybil resistance to be effective in a distributed threshold setting, the deduplication table $\mathcal{T}$ must be maintained consistently across all issuers. In our implementation, we leverage a permissioned distributed ledger shared among the threshold nodes to maintain an append-only record of all nullifiers. Before issuing a credential, each node verifies that the presented nullifier is not already in $\mathcal{T}$. 

The consensus protocol of the underlying ledger ensures that even in the presence of up to $t-1$ malicious issuers, the honest majority will maintain a consistent view of the nullifier list, preventing double-issuance attacks. This approach maintains privacy since the nullifiers themselves reveal no information about user identities, while ensuring system-wide agreement on uniqueness.
