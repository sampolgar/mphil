...
\begin{enumerate}
    ...
    \item Generate a pool of arbitrary size VRF key shares $\{\{s_2\}_i, \vk_{s_2}\}_m = \{\{s_2\}_i, \vk_{s_2}\}_1, \ldots, \{\{s_2\}_i, \vk_{s_2}\}_m$ for $i \in |n|$ and $m$ is arbitrary size. The pool can be replenished periodically by the committee.
    \item Returns $(\BG, \ck, \vk, \{\sk_i, \vk_i\}_{i \in [\ell]}, \{\{s_2\}_i\}_m$ for $i \in |n| )$
\end{enumerate}

Obtain, Issue
Obtain: user interacts to compute VRF key s then uses that in its credential request as one of the many commitments.
\begin{enumerate}
    \item User computes $s_1, r_s \sample \Z_p$, $\cm_{s_1} = \CMCom([s_1];r_s)$
    \item User interacts with the threshold to receive a subset of $\{s_{2_i}\}_{i \in S}$ shares $|S| \geq t$ and $\vk_{s_2}$, combines $s_2 \gets \mathsf{DKG.Combine}(\{s_{2_i}\}_{i \in S})$ and verifies $g^{s_2} = \vk$. 
    \item User computes $\cm_s = \CMCom([s_1 + s_2];r_s)$ and proof of correctness $\Pi^{\mathcal{R}_{\mathsf{DKG}}}(\cm_{s_1}, \cm_{s_2}, \cm_s, \vk_{s_2})$
\end{enumerate}
