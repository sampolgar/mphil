...
\begin{itemize}
    ...
    \end{enumerate}
      ...
      \end{enumerate}
      % We show that given a signature $\sigma = (h, h^{x+\sum_{k\in[\ell]} m_k y_k})$, we transform it into a rerandomizable signature in the non-threshold variant. Take $h_k = h^{y_k}$ and $h^{x+\sum_{k\in[\ell]}m_k} = h^x + h_k^{m_k}$ we produce $\ck' = $
