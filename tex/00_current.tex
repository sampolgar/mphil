...
\begin{definition}[MIMC-ABC System] A Multi-Issuer Multi-Credential Attribute-based Anonymous Credential system consists of the following $\PPT$ algorithms:
    ...
    \item $(\mathsf{Obtain}(\vec{m}, \usk, \opk, \aux), \mathsf{Issue}(\osk, \cm, \aux)) \rightarrow (\cred, \bot)$ is an interactive protocol between a user and an issuing organization. The user inputs their message vector $\vec{m} = [\id, \ctx, \attrs]$ containing a unique identifier $\id$ and context $\ctx$, along with secret key $\usk$. The user commits to their messages $\cm \gets \CMCom(\vec{m}; \usk)$ The issuer inputs their secret key $\osk$. The protocol outputs a credential $\cred$ containing $(\sigma, \cm)$ to the user and $\bot$ to the issuer.    
...
\begin{pchstack}
    ...
    \end{pcvstack}
     ...
     \procedure[]{$\mathrm{Game}^{\mathsf{\ANON}}_{\MIMCABC, \adv}(\lambda)$}{%
            ...
            \text{Return } (b' = b) \qquad \pclinecomment{Wins with the correct guess} \\
        }
    \end{pcvstack}
\end{pchstack}
...
\begin{pcvstack}[boxed, center, space=1em]
     \begin{pchstack}[space=1em]
         \procedure[]{$\mathrm{Game}^{\mathsf{\ANON}}_{\MIMCABC, \adv}(\lambda)$}{%
                ...
                \{\osk_j, \opk_j\} \gets \AdvA(\OrgKeyGen(\ppar)) \text{ for each issuer $j$}. \\
                \text{For $i$ in } \bit: \qquad \pclinecomment{$\AdvC$ initializes two honest users }\\
                \t \usk_i \gets \UserKeyGen(\ppar), \HU \gets \HU \cup \{i\}, \quad i \text{ has } \vec{m} \text{ such that } \phi(\vec{m}) = 1 \\
                \t \cm_i \gets \CMCom(\vec{m}_i; \usk_i),  \cred_i \gets \Issue(\osk, \cm_i) \\
                \AdvA^{\OHU, \OCU, \OOBTAIN, \OSHOW}(\{\osk_j, \opk_j\} ) \qquad \pclinecomment{Learning Phase} \\
                (i_0, i_1, \phi) \gets \AdvA() \qquad \pclinecomment{Challenge Phase}\\
                \text{Assert } i_0, i_1 \in \HU \setminus \CU, \quad \wedge \quad \phi(\vec{m}_{i_0}) = 1, \phi(\vec{m}_{i_1}) = 1 \\
