...
\subsection{Core Relations and $\Sigma$-protocol Implementation}
...
% Each relation is implemented as a standard 3-move Sigma protocol with:
% - Commitment: Prover selects random values r and computes first message t
% - Challenge: Verifier sends random challenge c
% - Response: Prover computes response z using witness and challenge

% The protocols provide special honest-verifier zero-knowledge and special soundness, 
% enabling our security proofs in Section 7.
...
% Identity binding ensures all credentials in a presentation belong to the 
% same identity. We leverage the position-binding property of our commitment 
% scheme from Section 3. Given commitments cm_j and cm_k with openings to 
% [id_j,ctx_j,attrs_j] and [id_k,ctx_k,attrs_k]:

% - The prover demonstrates id_j = id_k via a zero-knowledge proof of equality
% - Our position-binding guarantee ensures this equality cannot be forged
% - The security reduction (Section 7.1) shows breaking identity binding 
%   requires breaking the position-binding property

% 4.2 Credential Relationship Binding
% Beyond identity binding, our system can enforce structured relationships 
% between credentials through predicate φ. For example, requiring:
% - One credential derived from another (e.g., degree → professional license)
% - Consistency across credentials (e.g., name, date of birth)
% - Hierarchical verification (e.g., basic credential → extended privileges)

% These relationships are proven via the R_φ relation using standard techniques 
% for arithmetic circuits in Sigma protocols.

% 4.3 Protection Against Malicious Issuers
% We prevent malicious issuers from undermining anonymity or unforgeability 
% through a proof of correct key generation:

% R_verkey = {(vk,ck, (sk,x,{y_i})) | sk = g^x ∧ vk = g̃^x ∧ 
%                                   ∀i: g_i = g^{y_i} ∧ g̃_i = g̃^{y_i}}

% This ensures issuers cannot create keys with undisclosed structure that might 
% enable deanonymization or signature forgery.
...
We include an addition to the Rerandomizable Signature scheme algorithm 
\begin{itemize}
    \item $\mathsf{RS.VerKey}(\sk, \vk, \ck) \to \bit:$ verifies the correctness of the issuers secret and verification key $(\sk, \vk)$ and commitment key $\ck$:
        \[
        \mathcal{R}_{\mathsf{verkey}} = \{\pk = (\vk, \ck),(\sk, x, \{y_i\}_{i=1}^\ell) | sk = g^x \wedge vk = \tilde{g}^x \bigwedge_{i=1}^\ell (g_i = g^{y_i} \wedge \tilde{g}_i = \tilde{g}^{y_i})\}
...
We compared these algorithms for the most popular schemes, we show ours is sota
furthermore, we show our benefit in generating proof of knowledge is simple
