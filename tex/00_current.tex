...
\subsubsection{Security Properties}
In this section, we define the security properties of the MIMC-ABC system using game-based definitions. To model the adversary’s capabilities and the system’s state, we introduce the following lists and oracles:
...
The Credential is a signature $\sigma$ over commitment $\cm = \mathsf{CM.Com}([\id, \ctx, \attrs]; \usk)$ where $\attrs$ represents ancillary committed messages which we do not focus on in our protocol. 

Notes
\begin{itemize}
    \item $j$ is the index for issuers
    \item $i$ is the index for user's credentials
\end{itemize}
...
\begin{figure}
    ...
    \multicolumn{3}{l}{$\pirsok(\cred') = \zkpok\{(\id, \ctx,\usk') \; | \; \cmm' = \CMCom([\id, \ctx]; \usk') \wedge \RSVer(\sigma', \opk) = 1 \; \wedge \}$}\\[1em]
    $\underline{\mathsf{Show}(\cred, \phi)}$ && $\underline{\mathsf{Verify(\sigma', \cm', \pi, \opk)}}$ \\[1em]
    \multicolumn{3}{l}{Let $\cred = (\sigma, \cm, \usk, \opk)$, sample $\usk_\Delta, u_\Delta \sample \Z_p^2$}\\[1em]
    \multicolumn{3}{l}{$\sigma' = (\sigmaone^{u_\Delta}, (\sigmatwo \cdot \sigmaone^{\usk_\Delta})^{u_\Delta}) \gets \RSRand(\sigma, \usk_\Delta, u_\Delta)$}\\[1em]
    \multicolumn{3}{l}{$\cm' = (\cm \cdot g^{\usk_\Delta}) \gets \CMRand(\cm, \usk_\Delta)$, $\usk' = \usk + \usk_\Delta$}\\[1em]
    $\cred' = (\sigmam', \cm', \usk', u_\Delta)$ & $\xrightarrow{\pirsok(\cred')}$ & If $\pirsok(\cred')$ fails, return 0, else 1\\[1em]
    \end{tabular}
    \end{center}
